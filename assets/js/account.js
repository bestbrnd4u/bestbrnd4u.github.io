...
function subscribeIfFreshAccount(user) {
    ...
    // Разом із підпискою — і замовлення. Той самий момент: людина
    // щойно завела кабінет, і все, що вона замовляла до нього, має
    // повернутись у власну історію.
    claimGuestOrders().then(claimed => {

        if (claimed > 0 && user.id) loadOrders(user.id);
...
// -------------------------
// Замовлення без власника — назад у кабінет
//
// ЗВІДКИ ВОНИ БЕРУТЬСЯ. Людина замовляла гостем і тільки потім завела
// кабінет; або її обліковий запис видалили в панелі «Люди» — до
// міграції 037 замовлення при цьому зникали разом з акаунтом, тепер
// лишаються без власника.
//
// Хто саме замовляв, вирішує функція, а не сторінка: пошту вона бере
// з токена й вимагає, щоб та була підтверджена. Тут ми лише просимо
// подивитись і кажемо, скільки знайшлось.
//
// ПОМИЛКУ НЕ ПОКАЗУЄМО. Не знайшлося нічого — так і має бути в
// більшості випадків; збій функції теж не привід псувати людині
// реєстрацію повідомленням, якого вона не зрозуміє.
// -------------------------
...
async function claimGuestOrders() {
    ...
    const answer = await authCallFunction({
        site_action: "claim-orders",
        accessToken: token
    });

    return Number(answer?.data?.claimed) || 0;
...
// Самі кнопки й уся їхня поведінка — у assets/js/auth-widget.js:
// така сама картка входу стоїть на оформленні замовлення, і двома
// копіями цей код прожив би рівно до першої правки.
//
// Звідси лишається одне: куди повертатись після чужого акаунту. Для
// кабінету це сам кабінет.
authBindProviders(`${window.location.origin}/account`);

// Показуємо лише ті способи, які справді увімкнені в проєкті —
// подробиці й причина в auth-widget.js.
authShowEnabledProviders();

// Вхід через Telegram — теж у auth-widget.js. Тут лишається лише
// те, що відрізняє кабінет від оформлення: після входу сторінка
// перемальовує себе.
authBindTelegram(renderAuthState);

// Виклик нашої функції живе в auth-widget.js (authCallFunction). Ім'я
// коротше лишаємо: у цьому файлі воно зустрічається десяток разів.
const callFunction = authCallFunction;
...
resetPasswordForm?.addEventListener("submit", async event => {
    ...
    // Без current_password — і це навмисно, а не забуто. Сесію, яку
    // відкрив лист для відновлення, сервер помічає як recovery й
    // старого пароля в ній не питає навіть при увімкненому «Require
    // current password when updating». Людина ж сюди й прийшла тому,
    // що старого не пам'ятає: додати сюди це поле означало б зробити
...
// translateAuthError живе в assets/js/auth-widget.js: ті самі
// помилки показує ще й картка входу на оформленні замовлення.
...
// Що написати про заявку на відмову в деталях замовлення.
//
// «Решта замовлення лишається» правдиве лише для часткової відмови по
// ЖИВОМУ замовленню. У скасованому лишатись нічому — і ця фраза
// стояла прямо під бейджем «Скасовано», сперечаючись із ним у тій
// самій картці.
function refusalNoteText(order) {

    if (order.status === "cancelled" || isOrderFullyRefused(order)) {

        return "запит надіслано, менеджер зв'яжеться";

    }

    return "запит на частину товарів надіслано, решта замовлення лишається";
            ...
            <div class="order-card-details" hidden>
                ...
                <div class="order-detail-columns">
                    ...
                    <div class="order-detail-block">
                        ...
                        <p class="order-refusal-note"><strong>Відмова:</strong> ${refusalNoteText(order)}</p>
                        ` : ""}
                    </div>
...
function refusalWindow(order) {
    ...
    // СКАСОВАНЕ ЗАМОВЛЕННЯ ВІДМОВИ НЕ ПРИЙМАЄ.
    //
    // Воно не приїде — повертати нема чого. Заявка на нього означала б
    // для магазину роботу, якої не існує: подзвонити, з'ясувати, що
    // замовлення скасували ще тиждень тому, і закрити заявку ні з чим.
    //
    // Перевірка стоїть ПЕРШОЮ, і саме через це вона тут з'явилась:
    // скасоване замовлення НЕ доставляють, тож перевірка «доставлено»
    // нижче вважала, що строк ще не почався, — і кнопка «Відмова»
    // лишалась під товарами скасованого замовлення назавжди.
    if (order.status === "cancelled") return { allowed: false, reason: "cancelled" };
    ...
    const allowed = days < REFUSAL_DAYS;

    return {
        allowed,
        // Причина відмови в доступі — щоб і підпис під товаром, і
        // повідомлення при натисканні казали одне й те саме. Два
        // окремі списки причин розійшлися б.
...
// Чому відмовитись не можна — словами для покупця.
function refusalBlockedText(window_) {

    if (window_ && window_.reason === "cancelled") return "Замовлення скасовано — відмова не потрібна";

    return `Строк відмови (${REFUSAL_DAYS} днів) минув`;
...
function refusalMarkup(order, itemIndex) {
    ...
    if (!window.allowed) {
        ...
        // Кажемо ПРЯМО, чому кнопки немає, а не просто ховаємо її.
        //
        ...
        if (window.reason === "cancelled") {

            return `<span class="order-item-refuse-void">${refusalBlockedText(window)}</span>`;

        }

        return `<span class="order-item-refuse-expired"
                      title="Строк відмови рахується від дати доставки">
                    ${refusalBlockedText(window)}
                </span>`;
...
async function requestRefusal(button, itemIndex) {
    ...
    try {
        ...
        // Чи відмова взагалі приймається — перевіряємо ЩЕ РАЗ, а не
        // лише коли малюємо кнопку. І саме на замовленні, щойно
        // прочитаному з бази (вище), а не на тому, що показує сторінка.
        //
        ...
        // намалювали. Так само й скасування: замовлення міг скасувати
        // менеджер уже після того, як покупець відкрив кабінет. У всіх
        // цих випадках магазин отримував заявку, з якою нічого не
        // зробиш, і мусив пояснювати це покупцю вручну.
        const period = refusalWindow(order);

        if (!period.allowed) {

            showToast(refusalBlockedText(period));
...
profileForm?.addEventListener("submit", async event => {
    ...
    // ТЕЛЕФОН МІГ ЩОЙНО З'ЯВИТИСЬ — А ЗА НИМ І ЗАМОВЛЕННЯ.
    //
    // Людина могла замовляти гостем, назвавши лише телефон: пошти в
    // тому замовленні немає, і при реєстрації зіставити його не було
    // з чим. Момент, коли номер нарешті відомий, — саме цей.
    const claimed = await claimGuestOrders();

    if (claimed > 0) {

        showToast(claimed === 1
            ? "Знайшли ваше попереднє замовлення — воно вже в історії"
            : `Знайшли ваші попередні замовлення (${claimed}) — вони вже в історії`);
...
// пам'ятають.
//
// ЧОМУ ПИТАЄМО СТАРИЙ. За замовчуванням Supabase міняє пароль, маючи
// лише відкриту сесію, — старого не питає взагалі. Це означає, що
// будь-хто, хто сів за незаблокований комп'ютер, міняє пароль і
// забирає акаунт: власник більше не ввійде.
//
// Перевіряємо двічі, і друге не дублює перше:
//
//   1. Входом із тією ж поштою — тут, у браузері. Дає зрозумілу
//      відмову українською ще до того, як ми щось міняємо.
//   2. Полем current_password у самому запиті на зміну — його звіряє
//      вже сервер.
//
// Сама лише перевірка (1) не боронить нічого: хто дістався до
// відкритої сесії, звертається до API напряму й обходить її — наш
// код у цьому обміні не бере участі взагалі. Захищає саме (2), і
// вмикає її перемикач «Require current password when updating» у
// панелі: Authentication → Sign In / Providers → Email.
//
// Поле надсилаємо завжди, незалежно від перемикача: при вимкненому
// сервер його просто ігнорує. Тому порядок робіт саме такий — спершу
// цей код на прод, і лише потім перемикач. Навпаки означало б, що між
// перемикачем і викладкою зміна пароля відмовляє всім.
//
// Відновлення пароля листом (вище по файлу) цього не стосується:
// сесію з листа сервер помічає як recovery й current_password у ній
// не питає. Інакше й бути не могло — людина міняє пароль саме тому,
// що старого не пам'ятає.
// -------------------------
...
passwordForm?.addEventListener("submit", async event => {
    ...
    // current_password — та сама перевірка, але на сервері. Див.
    // пояснення на початку розділу.
    ...
    if (error) {
        ...
        console.warn("Не вдалося змінити пароль:", error.code || error.message);

        // Сервер міг відхилити старий пароль і сам: між нашим входом
        // вище й цим запитом пароль могли змінити з іншого пристрою.
        // Тоді чесна відповідь — та сама, що й там.
        passwordMessage.textContent =
