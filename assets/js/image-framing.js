...
(function (root) {
    ...
    function normalizeFrame(frame) {
        ...
        // Рішення про тло зберігаємо тут же, у кадрі фото.
        //
        // "white" — зробити білим примусово (навіть якщо автоматика
        //           вирішила, що чіпати не варто);
        // "keep"  — не чіпати (навіть якщо автоматика хотіла);
        // порожньо — як вирішить автоматика.
        //
        // Окремого поля в товарі не заводимо: рішення стосується
        // конкретного файлу, а кадр — уже саме такий запис «на файл».
        // Друге поле означало б два місця, які легко розсинхронити.
        // "cutout" — вирізати товар нейромережею й покласти на біле.
        // Потрібне там, де заливка безсила: знімок на столі, на мармурі,
        // з градієнтом (див. scripts/cutout.js).
        var bg = frame.bg === "white" || frame.bg === "keep" || frame.bg === "cutout"
            ? frame.bg
            : null;

        if (zoom === MIN_ZOOM && x === 50 && y === 50 && !bg) return null;

        var out = { zoom: zoom, x: x, y: y };
