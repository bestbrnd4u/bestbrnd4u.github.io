...
(function (root) {
    ...
    // ЗАЛИВКА Й ЦИФРИ ТАЙМЕРА — СПІЛЬНІ ДЛЯ ДВОХ НАБОРІВ.
    //
    // Це навмисний виняток із того, що набори незалежні, і різниця
    // між ним і скасованим злиттям важлива.
    //
    // Кольори ТЕКСТУ переносити не можна: на банері напис лежить на
    // фото й мусить бути світлим, на головній — на світлій сторінці й
    // мусить бути темним. Той самий колір в обох місцях означає, що в
    // одному з них напис зникає.
    //
    // А таймер — плашка з власним тлом. Вона виглядає однаково будь-де,
    // і задавати її двічі означає просто зайву роботу: власник
    // поставив #0d0101 на банері й побачив на головній червону — не
    // тому, що так хотів, а тому, що друге поле лишилось порожнім.
    //
    // Тому порожнє поле бере значення з сусіднього набору, а
    // заповнене — своє. Перенесення двостороннє: діє той колір, який
    // задали першим, байдуже в якому з наборів.
    function inheritTimer(style, other) {
        ...
        ["timerBg", "timerText"].forEach(function (key) {

            if (isColor(out[key])) return;
            if (!isColor(b[key])) return;
