...
async function initCatalog() {
    ...
    try {
        ...
        const [catalogData, live] = await Promise.all([
            loadCatalog(),
            window.LiveStock ? window.LiveStock.load() : Promise.resolve(null)
        ]);

        // Кожен колір — окрема картка. Сама функція живе в common.js:
        // те саме розгортання потрібне акціям (сторінка акції й ряд під
...
// лишилась на місці, можна додати другий або зняти обраний.
//
// АЛЕ НЕ НА КОЖНІЙ СТОРІНЦІ
// --------------------------
// Спершу смуга з'являлась у каталозі завжди — і в розділах теж. Вийшло
// навпаки задуманому: людина відкриває «Новинки», щоб подивитись
// товари, а над ними стоїть перелік із двадцяти однієї назви й відсуває
// сам каталог униз. Перелік потрібен тому, хто його шукає, а не всім.
//
// Тому смуга — на вимогу. Просить її позначка brands=1 в адресі, і
// ставить цю позначку рівно один пункт меню — «Усі бренди» в ховері
// розділу (див. assets/js/mega-menu.js). У «Каталозі» той самий пункт
// веде на /brands/, де перелік лежить у розмітці.
//
// Позначка переживає кліки сама: writeState() чіпає лише свої ключі, а
// чужі в адресі лишає як є.
//
...
// Смугу просять позначкою в адресі, а не налаштуванням у коді.
//
// Читаємо щоразу з location, а не кладемо в стан при завантаженні: це
// рівно одне звернення за весь показ сторінки, зате нема ще однієї
// змінної, яку доведеться тримати в синхроні з адресою.
function brandStripAsked() {

    return new URLSearchParams(location.search).get("brands") === "1";

}

function fillBrandStrip() {

    if (!brandStrip || hubBrandChips().length || !brandStripAsked()) return;
...
function render() {
    ...
    // Пошук, який нічого не знайшов — у журнал сайту.
    //
    // НАВІЩО. Магазин додає товари руками, і це єдине місце, де видно
    // попит, якого він не покриває: чи то товару немає, чи то він є,
    // але зветься інакше (тоді досить дописати написання в пошук).
    //
    // ТІЛЬКИ ПОШУК, не фільтри. Порожній результат після «Бренд +
    // ціна до 2000» — це не «немає такого товару», а «людина звузила
    // до нуля», і в журналі це був би шум.
    //
    // Умови записування (згода, нуль результатів, ніяких пошт і
    // чисел, лише дописаний запит) — у самій searchMiss().
    if (!list.length && window.ErrorReport && window.ErrorReport.searchMiss) {

        window.ErrorReport.searchMiss(search.value);
