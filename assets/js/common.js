...
function updateFavoriteButtons() {
    ...
    document.querySelectorAll(".favorite").forEach(button => {
        ...
        const on = isFavorite(id, color, size);

        button.classList.toggle("active", on);

        // СТАН — НЕ ЛИШЕ КОЛІР СЕРДЕЧКА.
        //
        // Заміряно на проді 25.09.2026: «уже в обраному» несло тільки
        // клас .active, тобто зафарбовану іконку. Хто не бачить
        // екрана — не міг дізнатись, чи товар уже в списку, а підказка
        // всім однаково обіцяла «Додати в обране» навіть тоді, коли
        // клац товар ПРИБИРАЄ. Кнопка казала неправду про те, що
        // зробить. WCAG 4.1.2 «Ім'я, роль, значення», рівень A.
        //
        // Ім'я кнопки лишається сталим («В обране: <назва>», його дає
        // шаблон картки), а стан несе aria-pressed — так це і мусить
        // працювати для перемикача. Підказка title іде за станом, бо
        // її бачить той, хто водить мишею, і їй теж не можна брехати.
        //
        // «✕ Видалити з обраного» на сторінці обраного пропускаємо: це
        // не перемикач, а одна дія, і назву вона має власну.
        if (!button.classList.contains("favorite-row-remove")) {
            button.setAttribute("aria-pressed", on ? "true" : "false");
            button.title = on ? "Прибрати з обраного" : "Додати в обране";
        }
...
// Пошук
//
// ЧОМУ НЕ ОДИН includes()
// ------------------------
// Саме так тут і було: увесь запит перевірявся як суцільний підрядок.
// Тобто слова мусили стояти в даних ПОРУЧ і в тому самому порядку —
// і «coach гаманець» не знаходило нічого, тоді як «гаманець coach»
// знаходило два товари. Так само вбивав пошук дефіс («окуляри ray ban»
// проти «ray-ban окуляри») і множина замість однини.
//
// Тепер запит розбивається на слова, і товар підходить, якщо
// знайдені ВСІ. Порядок і розділові знаки більше не важать.
// -------------------------

// Розбір запиту на слова. Розділювач — усе, що не літера й не цифра,
// тож «ray-ban» і «ray ban» дають однакові слова.
...
// Найкоротший корінь, до якого можна вкорочувати слово.
//
// Українська змінює переважно останні один-два символи («чорний» →
// «чорні», «окуляри» → «окуляр»), тому слово від п'яти символів
// шукаємо ще й у вкороченому вигляді. Коротше чотирьох не вкорочуємо:
// «сум» знайшлося б у «сумісний» і в половині описів.
const MIN_STEM = 4;

// Чи є це слово в тексті товару.
function hasSearchWord(haystack, word) {

    // Один-два символи збігаються всередині чужих слів: «мк»
    // знаходилось у «марк», і запит із двох літер давав 47 товарів.
    // Тому такі короткі шматки вимагаємо ЦІЛИМ словом — саме так у
    // ключових словах і лежать скорочення брендів («мк», «mj», «ax»).
    if (word.length <= 2) {

        const edge = "[^\\p{L}\\p{N}]";

        // Екранувати слово не потрібно: searchTokens() розбиває запит
        // по всьому, що не літера й не цифра, тож спецсимвол
        // регулярки в ньому опинитись не може за побудовою.
        return new RegExp(`(^|${edge})${word}(${edge}|$)`, "u").test(haystack);

    }

    if (haystack.includes(word)) return true;

    // Скільки символів можна відкинути.
    //
    // ЧОМУ НЕ ЗАВЖДИ ДВА. Спершу тут стояло «до двох», і на короткому
    // слові це різало не закінчення, а корінь: «лакост» ставало
    // «лако» — і запит «лакост» знаходив 35 товарів, першим із яких
    // ішов Marc Jacobs зі словом «Лаконічна» в описі.
    //
    // Тому два символи відкидаємо лише в слові від семи літер, де
    // після них лишається справжній корінь. Коротшому досить одного:
    // «чорні» → «чорн» знаходить «чорний», а «лакост» → «лакос» уже
    // ні з чим чужим не збігається.
    const maxCut = word.length >= 7 ? 2 : 1;
...
// Текст, по якому шукаємо. Один на весь сайт: раніше цей самий список
// полів був продубльований у каталозі, і дві копії неминуче
// розійшлися б.
...
function matchesQuery(product, q) {
    ...
    // Порожній запит нічого не фільтрує.
    if (!words.length) return true;

    const haystack = searchHaystack(product);

    return words.every(word => hasSearchWord(haystack, word));
...
// забути про друге.
// Те саме місце на клавіатурі в кирилиці й латиниці.
//
// НАВІЩО ЦЕ ВЗАГАЛІ ПОТРІБНО
//
// Бренди тут пишуться латиницею — Coach, Lacoste, Prada, — а
// українець набирає їх, не дивлячись на клавіатуру. Якщо розкладка
// лишилась кириличною, у поле падає «сщфср» замість «coach»: літери
// різні, клавіші ті самі.
//
// ЦЕ НЕ ПРИПУЩЕННЯ. У журналі сайту (site_issues, різновид
// search_miss) лежать справжні записи з 08.09.2026: запити «co» і
// «сщ» з однієї сторінки за три секунди. «сщ» — це рівно «co» на
// кириличній розкладці: людина почала писати Coach, побачила
// абракадабру й перемкнулась. Магазин при цьому показав їй «нічого
// не знайдено» — на бренд, якого в каталозі вісімнадцять моделей.
//
// Порядок рядків — фізичний ряд клавіш, тому таблиці й читаються
// поруч. Російські літери теж є: у половини людей стоїть саме та
// розкладка, і ъ/ї, ы/і, э/є ділять одну клавішу.
const KEYBOARD_ROWS = [
    ["йцукенгшщзхї", "qwertyuiop[]"],
    ["йцукенгшщзхъ", "qwertyuiop[]"],
    ["фівапролджє", "asdfghjkl;'"],
...
const LAYOUT_MAP = (() => {
    ...
    KEYBOARD_ROWS.forEach(([cyrillic, latin]) => {

        for (let i = 0; i < cyrillic.length && i < latin.length; i++) {

            // Перший запис виграє: «й» однакова в обох розкладках, а
            // от «ї» та «ъ» — різні літери на одній клавіші, і нам
            // потрібна українська.
            if (!(cyrillic[i] in map)) map[cyrillic[i]] = latin[i];
            if (!(latin[i] in map)) map[latin[i]] = cyrillic[i];
...
// Той самий набір клавіш іншою розкладкою. Порожньо, якщо міняти
// нічого: цифри, пробіли й уже правильний текст лишаються собою.
function swapLayout(text) {
    ...
    const out = source.replace(/./g, char => {

        const swapped = LAYOUT_MAP[char];

        if (swapped === undefined) return char;
...
function searchProducts(list, q) {
    ...
    const found = matchAll(all, q);

    if (found.length) return found;

    // Нічого не знайшли — можливо, справа в розкладці. Пробуємо ще
    // раз тими самими клавішами.
    //
    // САМЕ ЯК ЗАПАСНИЙ ШЛЯХ, а не завжди: «сом» латиницею це «cjv»,
    // і якби ми шукали обидва варіанти одразу, чесний український
    // запит іноді витягував би сміття. Спершу те, що людина
    // написала; перекладена розкладка — лише коли інакше порожньо.
    const other = swapLayout(q);

    return other ? matchAll(all, other) : [];

}

function matchAll(all, q) {
...
document.addEventListener("click", event => {
    ...
    if (colorBtn) {
        ...
        // Підпису кольору в картці більше немає (див. коментар на місці
...
const FALLBACK_SIZE_GROUPS = [
    ...
    // усіма товарами. Так само з годинниками.
    //
    // А ОКУЛЯРИ тут БУЛИ в тому самому переліку «теж ONESIZE» — і це
    // виявилось неправдою: у 7 із 17 стоять справжні 51, 53, 54
    // (ширина лінзи). Тобто фільтр за ними звужує з 17 до 7 —
    // звужує по-справжньому. Тому група з'явилась.
    //
    ...
    // Прив'язка САМЕ до категорії, не до розділу: у «Аксесуарах»
    // лежать ще гаманці й годинники, яким ці розміри не потрібні.
    { key: "eyewear", title: "Окуляри", department: "",
      categories: ["Окуляри і оправи"],
      sizes: ["51", "53", "54"] }
];
...
// ------------------------------------------------------
// Товари акції показуються у двох місцях: сітка на сторінці акції
// (promo.js) і ряд під банером на головній (app.js). Поки набір
// збирався окремо в кожному, копії розійшлись — на головній
// зберігався порядок адмінки, а на сторінці акції ні (про це
// promotionProducts вище). Розгортання по кольорах — рівно той самий
// випадок, тож обидва кроки стоять за одним викликом: сторінка не
// може забути другий.
//
// ПОРЯДОК КРОКІВ ВАЖЛИВИЙ. Спершу набір, потім розгортання:
// promotionProducts прибирає дублі за product.id, а після розгортання
// всі картки одного товару мають ОДИН і той самий id — тобто набір
// згорнув би їх назад в одну.
function promotionCards(promo, allProducts, departmentOf) {

    const set = promotionProducts(promo, allProducts, departmentOf);

    // Розгортаємо, поки прапорець в акції явно не вимкнули. Умова
    // саме така, а не `=== true`: у вже опублікованих акціях поля
    // немає зовсім, а сенс прапорця — щоб акція виглядала
    // заповненою, тож за замовчуванням він увімкнений.
    if (promo && promo.splitByColor === false) return set;

    return splitProductsByColor(set);

}

// Кожен колір — окрема картка.
//
// НАВІЩО
// -------
// Товар у двох кольорах займав ОДНУ картку: колір перемикався
// свотчами при наведенні. Список від цього виглядав порожнішим, ніж
// є, — покупець бачив 56 карток там, де насправді 70+ доступних
// варіантів. Тепер кожен колір показується своєю карткою.
//
// КОГО ЦЕ СТОСУЄТЬСЯ
// -------------------
// Каталог (catalog.js) розгортає весь список при завантаженні. Акції
// роблять те саме через promotionCards() вище — і на своїй сторінці,
// і в ряду під банером на головній.
//
...
// у даних лишається ОДИН, а список показує його стільки разів,
// скільки в нього кольорів. Ціна, опис і фото правляться в одному
// місці.
//
// Керується двома прапорцями, і обидва мусять дозволити:
//   • «Кожен колір — окрема картка в каталозі» у ТОВАРІ — вимикають,
//     коли кольори це відтінки одного й того самого (дві майже
//     однакові чорні);
