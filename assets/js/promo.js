...
// СТОРІНКА БЕЗ АКЦІЇ НЕ ПОВИННА ПОТРАПЛЯТИ В ІНДЕКС.
//
// Чому це не те саме, що звичайна 404: адреса з чужим або застарілим
// id віддає той самий шаблон із кодом 200. Для Google це не «сторінки
// немає», а ще одна сторінка з тим самим заголовком «Акція |
// BestBrnd4u» і тим самим описом «Акції та знижки BestBrnd4u» —
// тобто дубль. Таких адрес рівно стільки, скільки завершених акцій і
// одруків у посиланнях, а посилання на акцію живе в сторіс, у
// переписці й у чужих репостах набагато довше за саму акцію.
//
// Із sitemap завершені акції зникають самі (scripts/build-sitemap.js),
// але sitemap лише пропонує адреси — він не прибирає вже відомі.
//
// Так само поводиться сторінка товару, коли товар зняли з продажу:
// markProductPageNotFound у assets/js/product.js.
//
// follow, а не nofollow: сторінка порожня, але посилання на каталог
// на ній справжнє — хай пошуковик ним піде.
function showPromoNotFound() {

    document.getElementById("promoNotFound").hidden = false;

    // Заголовок вкладки лишався б «Акція | BestBrnd4u»: його міняє
    // renderPromoHero, а сюди ми дійшли повз нього. Те саме — в
    // закладці, в історії й у списку вкладок.
    document.title = "Акцію не знайдено | BestBrnd4u";

    setMetaByName("robots", "noindex,follow");

    setMetaByName("description",
        "Цю акцію вже завершено. Актуальні знижки — у каталозі BestBrnd4u.");
