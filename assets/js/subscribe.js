...
(function () {
    ...
    // ВІДПОВІДЬ ФОРМИ ТРЕБА НЕ ЛИШЕ ПОКАЗАТИ, А Й СКАЗАТИ.
    //
    // Рядок під формою — єдине, що повідомляє про результат: чи пішов
    // лист, чи адреса з одруком, чи бракує згоди. Хто не бачить
    // екрана, не дізнається про це ніяк: фокус лишається на кнопці,
    // сторінка не перезавантажується, більше нічого не змінюється.
    //
    // Роль ставимо ОДИН РАЗ при налаштуванні, а не в мить показу.
    // Читалка екрана має знати про живу область ДО того, як у ній
    // з'явиться текст, — інакше поява готового блока для неї
    // невідрізненна від звичайної перемальовки.
