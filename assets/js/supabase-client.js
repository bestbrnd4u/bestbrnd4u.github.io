...
// ЗВІДКИ ЛЮДИНА УВІЙШЛА — ЩОБ НЕ НАЗВАТИ ЧУЖИЙ СПОСІБ
//
// У кабінеті замість пошти стоїть рядок «Немає — ви увійшли через
// …». Спершу без пошти заходили тільки через Telegram, тож назва
// була зашита в текст. Тепер таких способів два:
//
//   Telegram   — пошти ніхто й не питав, у user.email лежить
//                службова адреса в нашому домені;
//   Facebook   — акаунт зареєстрований на телефон або людина зняла
//                галочку з пошти у вікні входу. Тоді email порожній.
//
// Другий випадок увімкнувся разом із перемикачем «Allow users without
// an email» у провайдері Supabase — без нього такі люди просто не
// могли увійти. І зашитий «Telegram» почав казати неправду.
//
// Telegram перевіряємо ПЕРШИМ і саме за доменом: акаунт створює наша
// ж функція через admin API, тож у app_metadata.provider там лежить
// «email», і провайдера звідти не видно.
function loginProviderName(user) {

    const email = String(user?.email ?? "").trim().toLowerCase();

    if (email.endsWith(TELEGRAM_EMAIL_DOMAIN)) return "Telegram";

    const provider = String(user?.app_metadata?.provider || "").toLowerCase();

    // Тільки ті, що справді ввімкнені або можуть бути ввімкнені:
    // вигадувати назви для решти немає сенсу — на невідоме є
    // запасний варіант у самому рядку.
    const names = {
        facebook: "Facebook",
        google: "Google",
        apple: "Apple"
