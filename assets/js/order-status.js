...
(function () {
    ...
    // Помилку треба СКАЗАТИ, а не лише намалювати.
    //
    // «Замовлення з таким номером і телефоном не знайдено» приходить
    // від сервера, коли фокус стоїть на кнопці «Перевірити». Сторінка
    // не перезавантажується, більше нічого не змінюється — той, хто
    // не бачить екрана, лишається думати, що кнопка не спрацювала.
    //
    // alert, а не status: людина щойно натиснула кнопку й чекає саме
    // цієї відповіді, тож перебити нею читання доречно.
    //
    // Роль ставимо тут, при налаштуванні, а не в showError: читалка
    // має знати про живу область ДО появи тексту.
    errorEl.setAttribute("role", "alert");

    // А знайдене замовлення віддаємо фокусу — див. renderOrder.
    ...
    function renderOrder(order) {
        ...
        // ФОКУС ТУДИ Ж — і це не те саме, що прокрутка.
        //
        // Картка знайшлась, але фокус лишився на кнопці «Перевірити»:
        // з клавіатури до накладної в ній треба протабати всю картку,
        // а читалка екрана взагалі не скаже, що щось з'явилось.
        //
        // Фокус на самій картці вирішує обидва: її прочитають, і
        // наступний Tab веде всередину, а не з початку сторінки.
