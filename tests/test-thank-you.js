...
console.log("\n[4] Знижка окремо від прохання про відгук");
{
    ...
    // СКІЛЬКИ САМЕ — рахуємо з коду, а не пишемо числом.
    //
    // ЧОМУ ЦЕ ВАЖЛИВО. Тут стояло жорстке 14, а поле в адмінці
    // дозволяло від 7 — і підказка там казала «не ставте менше» ніж
    // сім. Власник поставив 8, за правилами адмінки це було законно,
    // і CI впав (11.09.2026). Дві мої власні вимоги суперечили одна
    // одній, а розплачувався за це червоний деплой.
    //
    // Тепер число одне й береться зі скрипта прохання про відгук:
    // зсунеться той день — зсунеться й поріг.
    const reviewDays = Number(
        (read("scripts/request-reviews.js").match(/arg\("days",\s*"(\d+)"\)/) || [])[1]);

    check("день прохання про відгук прочитано з коду",
        reviewDays > 0, reviewDays);

    // Тиждень паузи. Знижка одразу після прохання про відгук
    // виглядає як плата за відгук — і це гірше за просто два листи
    // поспіль.
    const minDays = reviewDays + 7;

    check(`«дякуємо» не раніше ніж через ${minDays} днів`,
        settings.thankYouDays >= minDays,
        `зараз ${settings.thankYouDays}, прохання про відгук на ${reviewDays}-й день`);

    // І ГОЛОВНЕ: адмінка не має дозволяти те, що забороняє тест.
    // Саме ця щілина й спрацювала.
    const minInAdmin = Number(
        (admin.match(/name: "thankYouDays"[\s\S]{0,200}?min:\s*(\d+)/) || [])[1]);

    check("адмінка не дає ввести менше за той самий поріг",
        minInAdmin === minDays, `в адмінці min: ${minInAdmin}, треба ${minDays}`);

    check("в адмінці сказано, чому саме так",
        /плата за відгук/i.test(admin));
