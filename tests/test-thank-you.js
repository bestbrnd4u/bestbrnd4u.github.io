...
{
    // ЗА ЗАМОВЧУВАННЯМ — це про НОВУ установку, а не про поточний
    // вибір власника.
    //
    // Тут стояло settings.thankYou === false, тобто тест читав живе
    // значення з адмінки. Він і почервонів тієї ж хвилини, коли
    // власник увімкнув розсилку — саме те, заради чого вона зроблена
    // (11.09.2026). Це та сама помилка, що з порогом «дякуємо»: тест
    // не має контролювати рішення, які власник приймає в адмінці.
    //
    // Правило, яке справді треба стерегти: на чистій установці лист
    // не піде сам, поки не поставили галочку. Значить, дивитись треба
    // на default у самому полі.
    check("за замовчуванням вимкнено",
        /name: "thankYou"\n[\s\S]{0,200}?default: false/.test(admin));
...
console.log("\n[4] Знижка окремо від прохання про відгук");
{
    ...
    // ПРАВИЛО ЖИВЕ В ОДНОМУ МІСЦІ — і це не декорація.
    //
    // Спершу воно було записане двічі: поле в адмінці дозволяло від 7
    // днів, а тест вимагав не менше 14. Власник поставив 8 — законне
    // за адмінкою, заборонене тестом, — і CI впав (11.09.2026).
    // Правило, записане двічі, розходиться.
    //
    // Тепер обидва строки редагуються в адмінці, а перевіряє їх
    // scripts/letter-schedule.js — той самий модуль, який питає
    // скрипт перед відправкою.
    const { MIN_GAP_DAYS, schedule, tooClose } = require("../scripts/letter-schedule");

    const settings = JSON.parse(read("data/notifications.json"));

    check("обидва строки задаються в адмінці",
        Number(settings.reviewDays) > 0 && Number(settings.thankYouDays) > 0,
        JSON.stringify({ reviewDays: settings.reviewDays, thankYouDays: settings.thankYouDays }));

    const now = schedule(settings);

    check(`між листами щонайменше ${MIN_GAP_DAYS} днів`,
        tooClose(now.reviewDays, now.thankYouDays) === "",
        tooClose(now.reviewDays, now.thankYouDays));

    // ЧОМУ ВІДСТАНЬ, А НЕ «ПІЗНІШЕ». Знижка за день ДО прохання про
    // відгук виглядає так само куплено, як і за день після.
    check("надто близько — заборонено в обидва боки",
        tooClose(10, 14) !== "" && tooClose(20, 14) !== "",
        `${tooClose(10, 14)} / ${tooClose(20, 14)}`);

    check("рівно тиждень — уже можна", tooClose(7, 14) === "");

    // І ГОЛОВНЕ: скрипт мусить це питати ПЕРЕД відправкою, а не
    // покладатись на те, що в адмінці не помиляться. Поле в CMS не
    // вміє рахувати свій мінімум з іншого поля — тому останнє слово
    // там, де видно обидва числа.
    check("скрипт питає правило перед відправкою",
        /require\("\.\/letter-schedule"\)/.test(script)
        && /tooClose\(config\.reviewDays, config\.days\)/.test(script));

    check("і мовчки не шле, коли строки зійшлись",
        /const problem = tooClose[\s\S]{0,200}?if \(problem\) \{[\s\S]{0,120}?return;/.test(script));

    // Прохання про відгук теж бере строк з адмінки, а не з коду —
    // інакше редагувати можна було б лише половину пари.
    check("прохання про відгук бере строк звідти ж",
        /letter-schedule"\)\.schedule\(\)\.reviewDays/.test(read("scripts/request-reviews.js")));

    check("поле є в адмінці з поясненням",
        /name: "reviewDays"/.test(admin) && /плата за нього/i.test(admin));
