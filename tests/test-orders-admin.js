...
console.log("\n[4] Відмова: справжня заявка замість напису");
{
  ...
  // Раніше перевірявся підпис кнопки: button.textContent = "✓ Відмову
  // надіслано". Підпис живе до першого F5 — на перезавантаженій
  // сторінці кнопка поверталась, і покупець надсилав другу відмову на
  // той самий товар. Тепер результат малює сама картка, тож і
  // перевіряємо те, що лишається видимим: позначку замість кнопки.
  check("результат видно на місці кнопки, а не лише тостом",
        /done\.textContent = "✓ Відмову надіслано"/.test(ACC)
        && /order-item-refuse-done">✓ Відмову надіслано/.test(ACC));
  ...
  // Текст повідомлення переїхав із handleRefusal у formatRefusal
  // (format.js): там його ганяють тести на справжніх даних, а не
  // звіряють регуляркою. Тут лишається перевірка, що обробник таки
  // кличе саме його — і що в самому повідомленні є все, заради чого
  // менеджер його читає.
  const FMT = fs.readFileSync(
    path.join(ROOT, "supabase/functions/telegram-order-bot/format.js"), "utf8");

  check("обробник відмови будує повідомлення спільним форматером",
        /formatRefusal\(record, ord\)/.test(SRC));

  check("у сповіщенні є телефон клієнта", /formatRefusal[\s\S]{0,2000}tel:/.test(FMT));

  // Раніше тут були лише номер і сума: менеджер не знав, ЩО забирати,
  // і йшов звіряти зі списком у пошті.
  check("у сповіщенні є перелік товарів",
        /formatRefusal[\s\S]{0,2000}refused\.map\(itemLine\)/.test(FMT));

  check("у сповіщенні є сума до повернення",
        /formatRefusal[\s\S]{0,2000}до повернення/.test(FMT));
}
