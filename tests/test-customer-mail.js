...
console.log("\n[2] Лист про статус");
{
    ...
    // Gmail ховає «обрізаний вміст» — те, що повторює попередній лист
    // у тій самій темі (власник може виправити накладну й надіслати
    // лист удруге). Ріже він ХВІСТ, тому найважливіше мусить стояти
    // вище: спершу накладна, потім склад, і лише потім кнопка.
    const at = text => shipped.html.indexOf(text);

    check("відправлення: накладна й склад вище за кнопку",
        at("20450000000000") < at("Ваше замовлення")
        && at("Ваше замовлення") < at("Відстежити посилку"),
        `накладна ${at("20450000000000")}, склад ${at("Ваше замовлення")}, кнопка ${at("Відстежити посилку")}`);
