...
console.log("\n[15] Замовлення без власника вертаються в кабінет");
{
    // Людина замовляла гостем і завела кабінет пізніше; або її акаунт
    // видалили в панелі «Люди», і замовлення лишились без власника.
    // Обидва випадки закриває один маршрут — але просити його треба у
    // двох різних місцях, і саме це тут закріплено.
    check("запит на повернення є",
        /site_action: "claim-orders",\s*\n\s*accessToken: token/.test(js));

    check("пошту в запит не кладемо — її бере функція з токена",
        !/site_action: "claim-orders"[\s\S]{0,160}email/.test(js));

    // МІСЦЕ ПЕРШЕ: реєстрація. Пошта вже відома — за нею й шукаємо.
    check("просимо одразу після реєстрації",
        /claimGuestOrders\(\)\.then\(claimed =>/.test(js));

    // МІСЦЕ ДРУГЕ: збереження профілю. Замовлення гостя могло не мати
    // пошти взагалі — лише телефон, і при реєстрації зіставляти його
    // не було з чим. Момент, коли номер нарешті відомий, — саме цей.
    const at = js.indexOf("profileForm?.addEventListener");

    const save = at < 0 ? "" : js.slice(at, js.indexOf("\n// -------------------------", at));

    check("і після збереження профілю — там з'являється телефон",
        /const claimed = await claimGuestOrders\(\);/.test(save),
        save.slice(-160).replace(/\s+/g, " "));

    // Знайшли — показуємо. Мовчки підмінити список означало б, що
    // людина побачить чужі, як їй здасться, замовлення й не зрозуміє,
    // звідки вони.
    check("про знахідку кажемо вголос",
        /Знайшли ваше попереднє замовлення/.test(save));

    check("і перемальовуємо історію", /await loadOrders\(user\.id\);/.test(save));
}
