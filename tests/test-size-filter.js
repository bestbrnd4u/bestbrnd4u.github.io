...
console.log("\n[3] Результат на справжніх даних");
{
    ...
    //
    // Раніше розміри окулярів приходили у фільтр обхідним шляхом —
    // через «додаткові» (extra) під назвою розділу «Аксесуари», бо
    // власної групи в них не було. Тепер є, і це краще: у фільтрі
    // окрема група «Окуляри», а не домішка до гаманців і годинників.
    //
    // Тому правило про РЕЗУЛЬТАТ: розміри окулярів мусять бути у
    // фільтрі — байдуже, яким шляхом вони туди прийшли.
    const eyewearSizes = groups
        .filter(g => g.categories.includes("Окуляри і оправи"))
        .flatMap(g => g.sizes || [])
        .concat([...(extra.get("Аксесуари") || [])]);

    check("зʼявились розміри окулярів",
        eyewearSizes.includes("51"),
        eyewearSizes.join(","));
