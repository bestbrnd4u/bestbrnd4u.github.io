...
console.log("\n[8] Між пунктами меню немає смуги, де наведення гасне");
{
    // ЩО БУЛО. Меню відкривається чистим :hover, а між «Каталогом»,
    // «Новинками» й «Акціями» лежало по 38px порожнечі — це gap списку.
    // Щойно курсор заходив у цю смугу, панель закривалась і
    // відкривалась наново вже над наступним пунктом. Рух по діагоналі
    // вниз-убік панель просто гасив.
    //
    // Заміряно на живій сторінці ДО правки: Каталог 503-577, Новинки
    // 615-694, Акції 732-785 — три острови, і 71px між ними, де
    // наведено ні на що. ПІСЛЯ: 484-596, 596-713, 713-804, нуль.
    //
    // ЯК ЗРОБЛЕНО. Відступ між пунктами лишився той самий, але тепер
    // він НАЛЕЖИТЬ пунктам: половина зліва, половина справа. Від'ємне
    // поле повертає розкладку на місце. Так само на md-fashion.ua,
    // звідки й було прохання: там пункти йдуть впритул, а відстань між
    // написами дає padding усередині кожного.
    //
    // ПЕРЕВІРЯЄМО ПАРУ — саме вона може тихо розійтись. Лишити padding
    // без margin — пункти роз'їдуться вшир; лишити margin без padding —
    // зони наведення знову не зійдуться, і все повернеться. Обидва
    // мусять рахуватись від ОДНОГО числа, тому воно й назване.
    const css = fs.readFileSync(ROOT + "/assets/css/style.css", "utf8");

    const root = css.slice(css.indexOf(":root{"), css.indexOf("\n}", css.indexOf(":root{")));

    check("відстань між пунктами оголошена в :root",
          /--nav-gap:\s*\d+px/.test(root),
          "без цього її не знайде й перевірка втрачених змінних");

    const list = css.slice(css.indexOf("nav ul{"), css.indexOf("nav a{"));

    check("список бере відступ саме з неї",
          /gap:\s*var\(--nav-gap\)/.test(list),
          list.replace(/\s+/g, " ").slice(0, 100));

    const at = css.indexOf("nav li.has-mega > a{");
    const rule = at < 0 ? "" : css.slice(at, css.indexOf("}", at));

    check("правило пункту з меню знайшлось", rule !== "");

    check("пункт розтягнуто на пів відступу в кожен бік",
          /padding-inline:\s*calc\(var\(--nav-gap\)\s*\/\s*2\)/.test(rule),
          rule.replace(/\s+/g, " "));

    check("і повернуто від'ємним полем — розкладка не зрушила",
          /margin-inline:\s*calc\(var\(--nav-gap\)\s*\/\s*-2\)/.test(rule));

    // Панель починається рівно під шапкою (top:100%). Якщо зона
    // наведення нижча за саму шапку, між нею й панеллю лишиться смуга,
    // і меню гаситиме вже рух згори вниз — до панелі просто не дійти.
    check("зона наведення — на всю висоту шапки",
          /align-self:\s*stretch/.test(rule));
}
