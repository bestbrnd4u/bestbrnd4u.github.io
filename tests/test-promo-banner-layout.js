...
console.log("\n[2c] Кольори — у двох місцях і для всього, що заливається");
{
    ...
    // Обраний колір діє однаково до початку акції й поки вона йде —
    // тобто окремого правила для стану «іде» просто немає. Перевірку
    // «і воно теж фарбується» прибрано разом із правилом; що колір не
    // міняється сам, стереже перевірка в [2i] нижче.
...
console.log("\n[2i] Обраний колір ніщо не перебиває");
{
    ...
    // ТАЙМЕР НЕ МІНЯЄ КОЛІР САМ ПО СОБІ.
    //
    // Було правило [data-state="live"]{background:var(--danger)}:
    // відлік стояв темним до початку акції й ставав червоним, щойно
    // вона починалась. Власник прочитав це як поломку, і двічі —
    // підібрав заливку під банер, а вона змінилась сама.
    //
    // Кому потрібен червоний — ставить його полем «Колір таймера»,
    // і тоді він червоний завжди, а не половину акції.
    const liveColor = [...css.matchAll(/\[data-state="live"\]\s*\{([^}]*)\}/g)]
        .filter(m => /background|color/.test(m[1]));

    check("стан «іде» не перефарбовує таймер", liveColor.length === 0,
        liveColor.map(m => m[0].replace(/\s+/g, " ")).join(" | "));
}
