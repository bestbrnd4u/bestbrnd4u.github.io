...
console.log("\n[13] Заголовки складаються у зміст без дірок");
{
    ...
    [".footer h2", ".instagram-text h2", ".contact-card h2"].forEach(selector => {
        ...
        // [^{}]* між селектором і дужкою — бо селектор може бути
        // ЗГРУПОВАНИЙ: .instagram-text h2, .instagram-text h3{ … }.
        // Без цього перевірка казала «правила немає» саме тоді,
        // коли правило щойно зробили надійнішим.
...
console.log("\n[13b] Білий заголовок на градієнті лишається білим");
{
    // ЩО ЦЕ ЗАКРИВАЄ
    // ---------------
    // Блок «Стежте за нами в Instagram» лежить на синьо-фіолетовому
    // градієнті, і його заголовок білий. Білизна задавалась правилом
    // .instagram-text h3 — тобто була привʼязана до РІВНЯ заголовка.
    //
    // Коли заголовок підняли з h3 до h2 (щоб на сторінці не було
    // перескоку h1 → h3), на головній і сторінці товару тег змінили, а
    // в catalog.html і promo.html — ні. Правило перестало на них
    // діяти, і текст поїхав на глобальний темний колір.
    //
    // Заміряно 25.09.2026 на проді: контраст упав із 5.35-5.81 до
    // 3.05-3.32 — до самої межі норми для великого тексту. І це не
    // одна сторінка: catalog.html — шаблон для брендів, категорій і
    // розділів, тобто понад тридцять.
    //
    // Помилки не видно ні в розмітці, ні в CSS окремо: обидва
    // правильні, розходяться лише разом.
    const pages = fs.readdirSync(ROOT)
        .filter(file => file.endsWith(".html"))
        .filter(file => fs.readFileSync(path.join(ROOT, file), "utf8").includes("instagram-text"));

    const рівні = new Map();

    pages.forEach(file => {
        const hit = fs.readFileSync(path.join(ROOT, file), "utf8").match(/class="instagram-text">\s*<(h[1-6])/i);
        const рівень = hit ? hit[1].toLowerCase() : "немає заголовка";
        if (!рівні.has(рівень)) рівні.set(рівень, []);
        рівні.get(рівень).push(file);
    });

    check(`сторінок із блоком Instagram — ${pages.length}`, pages.length >= 3, pages.join(", "));

    check("рівень заголовка однаковий на всіх",
        рівні.size === 1,
        [...рівні.entries()].map(([k, v]) => `${k}: ${v.join("/")}`).join("; "));

    // Головне: колір належить КАРТЦІ, а не рівню. Доки правило
    // перелічує обидва рівні, зміна тега не може погасити білизну.
    const rule = css.match(/\.instagram-text h2,\s*\.instagram-text h3\{([^}]*)\}/);

    check("правило кольору не привʼязане до одного рівня",
        Boolean(rule),
        "селектор мусить перелічувати і h2, і h3");

    if (rule) {
        check("і воно робить текст білим",
            /color:\s*#fff/i.test(rule[1]),
            rule[1].replace(/\s+/g, " ").trim().slice(0, 50));
    }

    // Тло, заради якого все це: якщо градієнт колись прибрали, білий
    // текст став би невидимим — і перевірка вище сама б утратила сенс.
    check("картка Instagram і далі з темним градієнтом",
        /\.instagram-card\{[^}]*linear-gradient/.test(css));
}
