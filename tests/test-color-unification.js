...
console.log("\n[5] Скільки пунктів у фільтрі насправді");
{
    ...
    //
    // ПРАВИЛО ПРО РЕЗУЛЬТАТ, А НЕ ПРО ЧИСЛО. Тут стояло
    // «families.size <= 12» — і перевірка почервоніла в день, коли
    // власник додав фіолетову сумку: сімей стало 13. Нічого не
    // зламалось, просто в каталозі зʼявився колір, якого раніше не
    // було, а стеля була взята з поточного стану каталогу.
    //
    // Ціль тут інша: щоб фільтр був КОРОТШИЙ за перелік назв у
    // кілька разів. Її й перевіряємо. Верхня межа теж є, але
    // осмислена — довідник сімей у common.js: більше, ніж у ньому
    // оголошено, зʼявитись не може за побудовою, і якщо зʼявилось —
    // це вже справжня помилка.
    const declared = COLOR_FAMILY_ORDER.length;

    check(`пунктів у фільтрі в кілька разів менше, ніж назв (${names.size} → ${families.size})`,
        families.size * 3 <= names.size,
        `${families.size} сімей на ${names.size} назв`);

    check(`сімей не більше, ніж оголошено в довіднику (${declared})`,
        families.size <= declared,
        `${families.size} проти ${declared}`);
