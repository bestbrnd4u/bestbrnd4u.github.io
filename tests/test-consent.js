...
console.log("\n[4] Політика описує те, що є в коді");
{
    ...
    check("сказано, що персональних даних у сам піксель не йде",
        /Персональних даних у сам піксель ми не передаємо/.test(policy));

    // ЩО ЗМІНИЛОСЬ. Раніше тут перевірялось речення «Персональних
    // даних ми в піксель не передаємо» — і воно було правдою, поки
    // Meta дізнавалась про покупки лише з браузера.
    //
    // Тепер про покупку повідомляє ще й сервер (Conversions API), і в
    // цьому повідомленні йде НЕЗВОРОТНИЙ ВІДПЕЧАТОК пошти, телефону,
    // імені та міста — інакше Meta не має до кого приписати покупку.
    // Отже старе речення стало б неправдою, а неправда в політиці
    // конфіденційності гірша за відсутність розділу.
    //
    // Тому перевіряємо, що виняток описаний прямо: що саме йде, у
    // якому вигляді й що без згоди не йде нічого.
    check("серверна подія описана окремо",
        /з нашого сервера/i.test(policy));

    check("сказано, що йде відпечаток, а не самі дані",
        /незворотний відпечаток/i.test(policy) && /SHA-256/.test(policy));

    check("сказано, що без згоди на рекламу цього немає",
        /без вашої згоди[\s\S]{0,120}немає/i.test(policy));
