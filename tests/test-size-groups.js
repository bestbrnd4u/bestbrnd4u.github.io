...
console.log("\n[2] Файл налаштувань коректний");
{
  ...
  // Сумки й годинники виведені окремо: усі вони мають ONESIZE, тож
  // фільтр за розміром для них нічого не звужував би.
  //
  // ОКУЛЯРИ БУЛИ в тому самому переліку «теж ONESIZE» — і це
  // виявилось неправдою: у 7 із 17 стоять справжні 51, 53, 54
  // (ширина лінзи). Фільтр за ними звужує з 17 до 7, тобто звужує
  // по-справжньому — тому група з'явилась.
  //
  // Перевіряємо СКЛАД, а не кількість: число саме по собі нічого не
  // стереже, а склад — стереже.
  const keys = GROUPS.map(g=>g.key).sort().join(",");

  check("групи ті, що очікуються", keys==="backpacks,clothes,eyewear,shoes", keys);
  GROUPS.forEach(g=>{
