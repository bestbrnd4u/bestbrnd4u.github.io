...
console.log("\n[2] Джерела картинок");
{
    ...
    // ЛІЧИЛЬНИКИ ШЛЮТЬ ДАНІ НЕ ЛИШЕ ЗАПИТАМИ, А Й КАРТИНКАМИ.
    //
    // Це друга така помилка в цьому ж місці, і механізм у неї той
    // самий, що з фото відгуків: джерело дозволене там, де його
    // помітили, і не дозволене там, де воно теж потрібне.
    //
    // GA4 вантажить бібліотеку скриптом (script-src) і шле події
    // запитом (connect-src) — обидва були дозволені. А частину даних
    // він надсилає КАРТИНКОЮ на googletagmanager.com/td, і цього в
    // img-src не було.
    //
    // Спіймано в консолі на проді 24.09.2026, наступного дня після
    // вмикання бойового режиму:
    //
    //   Loading the image 'https://www.googletagmanager.com/td?…&rtg=…'
    //   violates … "img-src 'self' data: …". The action has been blocked.
    //
    // Судячи з rtg= в адресі, це шлях ремаркетингу — дані для
    // аудиторій Google Ads просто не доходили.
    //
    // ЧОМУ ЗАМІР ПЕРЕД ВМИКАННЯМ ЦЬОГО НЕ ПОБАЧИВ. Маячок спрацьовує
    // не на кожному завантаженні: у восьми сторінках чистої перевірки
    // він не з'явився жодного разу. Тому перевірка тут — за списком
    // директив, а не за спостереженням.
    check("GA4 дозволений і як джерело картинок",
        allows("img-src", "www.googletagmanager.com"),
        (directives.get("img-src") || []).join(" "));

    check("і google-analytics теж — на випадок запасного шляху",
        allows("img-src", "www.google-analytics.com"),
        (directives.get("img-src") || []).join(" "));
    ...
    ["www.googletagmanager.com", "www.google-analytics.com"].forEach(host => {
        ...
        check(`${host}: дозволений і запитом, і картинкою`,
            !asConnect || allows("img-src", host));
