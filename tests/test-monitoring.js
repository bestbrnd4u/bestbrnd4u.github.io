...
//    параметрів адреси (у ?search= буває пошуковий запит).
//
//    ОДИН СВІДОМИЙ ВИНЯТОК: пошук, який не знайшов НІЧОГО,
//    записується разом із запитом — це журнал попиту, і без самого
//    запиту він безглуздий. Умови, за яких це лишається прийнятним
//    (згода, нуль результатів, ні пошт, ні телефонів), перевіряє
//    окремий набір: tests/test-search-miss.js.
...
{
    // Шлях сторінки — без параметрів. Це лишається правдою й після
    // появи searchMiss(): запит туди потрапляє окремим полем і лише
    // за умовами, а зі САМОЇ адреси ми його як раніше не беремо.
...
console.log("\n[9] Нерозгорнута функція теж помітна");
{
    // РЕГРЕСІЯ, ЯКУ ЦЕ ЛОВИТЬ. Нова можливість функції жива лише
    // після ДВОХ дій власника: міграція в базі й перерозгортання
    // функції. Пропустити друге легко — код у гілці є, тести зелені,
    // а на сайті мовчок.
    //
    // Заміряно: маршрут checkout-draft на проді відповідає «ignored»,
    // тобто розгорнута функція про нього не знає. Точно так само
    // колись виявились відгуки — випадково.
    const script = read("scripts/monitor.js");

    check("моніторинг питає саму функцію", /functions\/v1\/telegram-order-bot/.test(script));

    check("питає про незавершене оформлення",
        /functionRoute\("checkout-draft"/.test(script));

    // Питаємо навмисно недійсними даними: у базу нічого не пишеться й
    // лічильник звернень не витрачається.
    check("перевірка нічого не зберігає",
        /functionRoute\("checkout-draft", \{ email: "", items: \[\] \}\)/.test(script));

    check("публічним ключем, а не службовим",
        !/SERVICE_ROLE/.test(script.slice(script.indexOf("async function functionRoute"),
            script.indexOf("async function dnsTxt"))));

    check("у повідомленні сказано, що саме зробити",
        /перерозгорнуто після міграції 020/.test(script));
}

console.log("\n[10] Кеш: правила в Cloudflare не забуті");
{
    // НАВІЩО ЦЕ В МОНІТОРИНГУ. Це єдиний пункт розбору, який
    // робиться не в коді, а в панелі Cloudflare — і саме тому про
    // нього легко забути назавжди. Доки правил немає, перевірка
    // щодня нагадує (той самий підхід, що з DMARC).
    //
    // Заміряно на проді: файли коду з відбитком у адресі
    // (style.css?v=…) віддаються з max-age=14400 — чотири години
    // замість року, хоч така адреса не може стати неправильною
    // ніколи. А HTML край не кешує взагалі (DYNAMIC): кожен показ
    // іде до GitHub Pages.
    const script = read("scripts/monitor.js");

    check("моніторинг дивиться заголовки", /async function headers\(url\)/.test(script));

    check("перевіряє max-age файлів коду", /function maxAge\(value\)/.test(script));

    // Рік — саме тому, що адреса несе відбиток. Місяць у перевірці —
    // із запасом, щоб не червоніти через дрібну різницю налаштувань.
    check("поріг — не менше місяця", /age >= 2592000/.test(script));

    check("адресу файлу бере з живої сторінки, а не вгадує",
        /home\.body\.match\([\s\S]{0,120}\?v=/.test(script));

    check("перевіряє, що край кешує сторінки",
        /cf-cache-status/.test(script) && /DYNAMIC/.test(script));

    check("у повідомленні сказано, де це налаштувати",
        /docs\/КЕШ\.md/.test(script));

    // Тільки на бойовому: перед девом Cloudflare Pages зі своїми
    // правилами.
    check("на деві не перевіряємо",
        /кеш на краю[\s\S]{0,400}if \(INDEXABLE\)/.test(script));

    check("документація з точними налаштуваннями є",
        fs.existsSync(path.join(ROOT, "docs/КЕШ.md")));

    const doc = read("docs/КЕШ.md");

    check("у ній є вираз правила для файлів",
        /http\.request\.uri\.query contains "v="/.test(doc));

    check("і сказано виключити адмінку", /admin/.test(doc));

    // Дані про залишки НЕ кешуються надовго навмисно: товар, якого
    // немає, показаний як «в наявності», коштує дорожче за
    // зекономлені запити.
    check("попереджає не кешувати залишки надовго",
