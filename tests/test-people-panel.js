...
console.log("\n[N+2] Підписаному не пропонуємо підписатись");
{
    ...
    // Перевірку перенесено в саму remember(): там вона одна на всі
    // місця запису, а їх стало три. Розкидана по місцях виклику, вона
    // вже один раз розійшлась — і кеш почав брехати.
    check("збій мережі не вважається станом людини",
        mod.lookupState(502, null) === "unknown"
        && /function remember\(email, state\) \{\s*\n\s*if \(state === "unknown"/.test(client));
    ...
    // ЩО САМЕ ЗЛАМАЛОСЬ ЧЕРЕЗ ЦЕЙ КЕШ.
    //
    // Людина підписалась (стан «не підтверджено»), перейшла за
    // посиланням із листа — у MailerLite стала підписаною. В адмінці
    // стояло «Підписаний», а кабінет ще добу писав «підписку ще не
    // підтверджено»: запам'ятали стан, який існує рівно до натискання
    // посилання.
    check("проміжний стан не запам'ятовується",
        /if \(state === "unknown" \|\| state === "unconfirmed"\) return;/.test(client));

    // Панель — місце, де людина ПРИЙМАЄ РІШЕННЯ, і вчорашній знімок
    // тут не годиться. На решті сторінок кеш доречний: там від нього
    // залежить лише зайва пропозиція підписатись.
    check("панель налаштувань завжди питає наживо",
        /if \(known && !settings\) return;/.test(client));

    // Дзеркало того самого бага: підписався у футері, підтвердив — а в
    // пам'яті лишилось «не підписаний», і пропозиція висіла ще добу.
    check("після підписки пам'ять скидається",
        /function forget\(\)/.test(client)
        && /forget\(\);[\s\S]{0,200}form\.reset\(\);/.test(client));

    // «Ви вже підписані» малювалось червоним — стилем помилки. Нічого
    // не зламалось: галочка просто вже стоїть так, як її зберегли.
    check("«нічого не змінилось» не виглядає збоєм",
        /"Ви вже підписані" : "Ви й так не підписані", "ok"/.test(client));
