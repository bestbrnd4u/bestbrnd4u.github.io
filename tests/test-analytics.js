...
console.log("\n[4] Події розставлені в коді магазину");
{
    ...
    // ГОЛОВНЕ: рядки кошика мусять знаходитись.
    //
    // Тут стояло findCachedProduct(line.id) — кеш каталогу, який на
    // сторінці оформлення ПОРОЖНІЙ. Усі рядки відпадали на
    // filter(x => x.product), і події begin_checkout та purchase не
    // надсилались зовсім.
    //
    // Помітити це було майже неможливо: у звітах просто не було
    // покупок, а виглядало як «ще ніхто не замовляв».
    check("рядки беруться з getCartLines()",
        /const lines = getCartLines\(\);/.test(checkout));
    check("кеш каталогу тут не використовується",
        !/findCachedProduct\(line\.id\)/.test(checkout.replace(/\/\/[^\n]*/g, "")));
