...
console.log("\n[3] Що показує віджет — на живих даних");
{
    ...
    // У тексті ЛИШЕ посилання, без вітання й назви.
    //
    // Кирилиця в адресі кодується: кожна літера стає девʼятьма
    // символами. З «Доброго дня! Хочу замовити: <назва>» посилання
    // виходило 451 символ нечитабельного %D0%94%D0%BE… — таке не
    // вставиш ні в пост, ні в шапку профілю.
    check("у повідомленні саме посилання", text === productRow.value, text.slice(0, 60));
    check("вітання прибрано", !/Доброго дня/.test(text));

    // Головне: посилання мусить лишатись придатним для вставляння.
    check(`довжина посилання — ${telegramRow.value.length} символів`,
        telegramRow.value.length < 200, telegramRow.value.length);
