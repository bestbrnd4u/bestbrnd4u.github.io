...
console.log("\n[6] На сайті видно номер, а не порожнє місце");
{
    ...
    // Рядок став КНОПКОЮ «скопіювати артикул»: на телефоні виділити
    // «28-1» усередині тексту майже неможливо, а саме цим номером
    // покупець питає про товар — і саме за ним працює пошук по сайту.
    // Перевіряємо не тег, а те, що важливо: підпис на місці, номер
    // окремим вузлом, а копіюється сам код, а не рядок із підписом.
    check("під назвою — підписаний артикул, без бренду",
        /data-product-sku-text>\$\{activeSku \? `Артикул: \$\{escapeHtml\(activeSku\)\}`/.test(product));

    check("артикул можна скопіювати одним дотиком",
        /class="meta-chip sku-copy"/.test(product)
        && /data-sku="\$\{escapeHtml\(activeSku \|\| ""\)\}"/.test(product));
    ...
    check("посилання на бренд над заголовком лишилось",
        /<a class="brand\$\{[^}]*\}" href="\$\{brandHref\(product\.brand\)\}"/.test(product));

    // Підпис пише й обробник свотча: інакше при перемиканні кольору
    // «Артикул:» зникав, і лишався голий номер.
    //
    // Пише він тепер у ВЛАСНИЙ вузол тексту, а не в саму кнопку:
    // textContent на кнопці стер би значки копіювання разом із
    // розміткою.
    check("при перемиканні кольору підпис не зникає",
        /label\.textContent = sku \? `Артикул: \$\{sku\}` : ""/.test(common));

    check("і код для копіювання оновлюється разом із ним",
