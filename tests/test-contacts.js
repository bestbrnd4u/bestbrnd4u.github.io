...
{
  // Перевіряємо ПОСИЛАННЯ, а не саме слово.
  //
  // Мета перевірки — щоб не лишилось мертвих посилань на неіснуючу
  // сторінку. А згадка в тексті буває доречною: у політиці
  // конфіденційності прямо сказано, що пікселя Facebook на сайті
  // НЕМАЄ. Заборона на слово змушувала б викидати саме ту фразу,
  // заради якої політику й читають.
  const withFb = pages.filter(f =>
      /(?:href|src)="[^"]*facebook\.com/i.test(read(f)));

  check("немає посилань на Facebook (сторінки не існує)",
        withFb.length === 0, withFb.join(", "));
