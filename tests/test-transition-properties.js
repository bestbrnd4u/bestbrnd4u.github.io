...
console.log("\n[4] Тривалості: реакція на дію — не довше 300ms");
{
    // Понад 300ms UI-перехід починає читатись як затримка. Винятки —
    // рух на велику відстань, де 350ms саме те, що треба.
    const ДОВГІ_НАВМИСНО = {
        ".gallery-slide": "слайд галереї їде на всю ширину — це переміщення, не реакція на натискання",
        ".lightbox-track": "доріжка лайтбокса гортає фото на весь екран",
        ".gallery-photo": "клік-зум фото: навмисне повільне розкриття"
    };
    ...
    for (const t of переходи) {
        ...
        if (макс <= 300) continue;
        if (ДОВГІ_НАВМИСНО[t.sel]) continue;
        порушники.push(`${t.sel} (рядок ${t.line}): ${макс}ms`);
    }
    check("немає UI-переходів довше за 300ms",
        порушники.length === 0, prettify(порушники));

    // Винятки мусять лишатись реальними: якщо правило перейменували,
    // запис у списку тихо перестає діяти й перевірка вище це проґавить.
    const відсутні = Object.keys(ДОВГІ_НАВМИСНО)
        .filter(sel => !переходи.some(t => t.sel === sel));
    check("усі записані винятки ще існують у стилях",
        відсутні.length === 0, відсутні.join(", "));
}
