...
console.log("\n[9] Дата очікуваної наявності для «під замовлення»");
{
    ...
    //
    // Розбір поля переїхав до assets/js/product-offer.js: той самий
    // строк тепер потрібен і розмітці сторінки, яка раніше обіцяла
    // Google 2–5 днів на товар, що йде 10–14. Тому перевіряємо, що
    // фід бере помічника ЗВІДТИ, а не завів собі другий.
    const offerSrc = read("assets/js/product-offer.js");

    check("строк — із поля товару", /preOrderDays/.test(offerSrc));

    check("фід бере розбір зі спільного модуля",
        /preOrderWorkDays \} = require\("\.\.\/assets\/js\/product-offer\.js"\)/.test(feedSrc)
        || /preOrderWorkDays.*require\("\.\.\/assets\/js\/product-offer\.js"\)/.test(feedSrc));

    check("власної копії розбору у фіді не лишилось",
        !/function preOrderWorkDays/.test(feedSrc));

    // «10-14 робочих днів» → 14. Обіцяти коротший строк, ніж буває,
    // гірше, ніж не обіцяти нічого. Для ДАТИ очікування береться
    // саме більше число; у розмітці сторінки стоїть повний діапазон
    // (10–14), бо там це те саме, що читає покупець.
    check("з діапазону береться довший строк",
        /return range \? range\.max : 0;/.test(offerSrc));
