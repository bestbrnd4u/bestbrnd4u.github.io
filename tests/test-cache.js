...
console.log("\n[2] Документ описує правило, яке не чіпає дані");
{
    ...
    // ДРУГА РЕГРЕСІЯ ТОГО САМОГО ВИРАЗУ: правила кешу діють на всю
    // зону, тож без умови домену під двогодинний кеш потрапляв ще й
    // dev.bestbrnd4u.com. А чистка кешу стоїть лише в продовій
    // виливці — dev не чистить ніхто.
    //
    // Заміряно: dev/catalog → cf-cache-status: HIT, Age: 120,
    // max-age=7200. Тобто правку залито, збірка пройшла, Pages
    // опублікував — а на dev до двох годин стара сторінка. Виглядає
    // рівно як «зміни не поїхали».
    check("правило сторінок обмежене основним доменом",
        /http\.host eq "bestbrnd4u\.com"/.test(doc));

    check("сказано, чому в виразі домен", /Чому в виразі є домен/.test(doc));

    // І людині треба знати, як подивитись зміни, не чекаючи дві
    // години: випадковий параметр в адресі дає інший ключ кеша.
    check("описано, як перевірити dev одразу",
        /Як подивитись зміни на dev одразу після заливки/.test(doc)
        && /cf-cache-status/.test(doc));

    // Найчастіша хибна порада — «оновіть сторінку». Кеш стоїть на
    // краю, і Ctrl+F5 його не чіпає.
    check("сказано, що Ctrl+F5 не допомагає", /Ctrl\+F5 тут \*\*не допомагає\*\*/.test(doc));

    // Збірка dev робить ДРУГИЙ коміт у гілку, тож Pages публікує
    // двічі — між публікаціями видно напівзібраний стан.
    check("попереджено про подвійну публікацію",
        /сам робить другий коміт/.test(doc));
