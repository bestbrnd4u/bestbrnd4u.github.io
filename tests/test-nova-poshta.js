...
console.log("\n[6b] Підказки не тільки видно — їх ще й чути");
{
    // ЩО БУЛО НЕ ТАК. Стрілки й Enter працювали (див. [6]), тобто з
    // клавіатури місто вибрати було можна. Але екранний читач про
    // список не знав нічого: між полем і <div> із підказками не
    // існувало жодного звʼязку. Людина друкує «Льв», під полем
    // беззвучно зʼявляються девʼять варіантів, вона тисне стрілку —
    // і чує тишу. І це сторінка оформлення, через яку проходить
    // кожне замовлення.
    check("поле оголошене як combobox",
        /setAttribute\("role", "combobox"\)/.test(page));

    check("і каже, що підказки будуть списком",
        /setAttribute\("aria-autocomplete", "list"\)/.test(page));

    check("список оголошений як listbox",
        /box\.setAttribute\("role", "listbox"\)/.test(page));

    check("поле вказує на свій список",
        /setAttribute\("aria-controls", box\.id\)/.test(page));

    // id мусить бути свій у кожного списку: полів із довідником на
    // сторінці кілька (відділення, поштомат, адреса кур'єра).
    check("у кожного списку власний id",
        /listCount \+= 1;/.test(page) && /box\.id = "np-suggest-" \+ listCount/.test(page));

    check("розкритий список видно з поля",
        /setAttribute\("aria-expanded", "true"\)/.test(page)
        && /setAttribute\("aria-expanded", "false"\)/.test(page));

    // Пункти — з роллю й id, інакше на них немає на що вказати.
    check("пункти оголошені як option",
        /role="option"/.test(page) && /id="' \+ state\.box\.id \+ "-" \+ index \+ '"/.test(page));

    // Фокус лишається в полі, «обраність» мандрує — це і є
    // aria-activedescendant.
    check("поточний пункт названо через activedescendant",
        /setAttribute\("aria-activedescendant", current\.id\)/.test(page));

    check("і знімається, коли список закрився",
        /hide\(state\)[\s\S]{0,400}removeAttribute\("aria-activedescendant"\)/.test(page));

    // Пункти — <button>, тобто самі собою потрапляли в таб-порядок:
    // щоб дійти від міста до наступного поля, довелось би протиснути
    // Tab девʼять разів.
    check("пункти прибрані з таб-порядку", /tabindex="-1"/.test(page));

    // І САМ СПИСОК — ТЕЖ.
    //
    // У нього max-height і overflow-y:auto, тобто це смуга з
    // прокруткою. Chrome від 127-ї версії робить такі смуги
    // зупинкою таба, коли всередині немає нічого фокусованого, — щоб
    // їх можна було прокрутити з клавіатури. Доки пункти були
    // звичайними кнопками, умова не виконувалась; щойно ми прибрали
    // їх із таб-порядку, Tab почав потрапляти в порожній контейнер.
    // Спіймано в браузері, бо в розмітці все виглядало правильно.
    check("порожній контейнер не стає зупинкою таба",
        /box\.setAttribute\("tabindex", "-1"\)/.test(page));

    // Без коментарів: усередині самого правила лежить довге
    // пояснення про margin-top, і воно розсовує вікно пошуку.
    check("і в стилях він справді смуга з прокруткою",
        /\.np-suggest\{[\s\S]{0,300}overflow-y:auto/.test(
            read("assets/css/style.css").replace(/\/\*[\s\S]*?\*\//g, "")));

    // Підсвітка й aria-selected мусять мінятись разом — інакше око
    // бачить один пункт, а читач називає інший.
    check("підсвітка й озвучення йдуть разом",
        /function markActive\(state\)[\s\S]{0,400}classList\.toggle\("active", on\)[\s\S]{0,200}setAttribute\("aria-selected"/.test(page));

    check("стрілки кличуть саме її",
        /state\.active = \(state\.active \+ step[\s\S]{0,120}markActive\(state\);/.test(page));
}
