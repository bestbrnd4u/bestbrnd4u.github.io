...
console.log("\n[3] Шляхи не ламаються на глибині /p/<slug>/");
{
    ...
    check("стилі підключені з кореня", /href="\/assets\/css\/style\.css"/.test(html));
    check("product.js підключений з кореня", /src="\/assets\/js\/product\.js"/.test(html));
}
...
console.log("\n[7] Стара адреса ?id= не ламається, а веде на канонічну");
{
    ...
    check("product.js тягне каталог з кореня, а не відносно теки",
        /fetch\("\/data\/products\.json"\)/.test(productJs));
}
