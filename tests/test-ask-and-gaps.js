...
//    просто числа.
//
// 3. НЕЗАПОВНЕНІ ПОЛЯ КАРТОЧОК. Було 37 товарів без габаритів і 38
//    без матеріалу — і дізнатись про це не було звідки. Обидва поля
//    показуються покупцеві й ідуть у фід Google Shopping.
//
//    Заповнено з паспортів виробників: матеріал є в усіх 103
//    товарах, габарити — скрізь, крім двох сумок Armani Exchange
//    (складські артикули, моделей уже немає на сайті бренду).
//
//    У кросівок габаритів немає НАВМИСНО: їх заміняє розмірна
//    сітка, і жоден виробник взуття їх не публікує. Тому звіт про
//    габарити взуття не питає — інакше десять кросівок висіли б у
//    попередженні вічно й привчали б його гортати.
const fs = require("fs");
...
console.log("\n[4] Незаповнені поля карточок видно в журналі збірки");
{
    ...
    // ПОВЕДІНКА, а не вигляд коду. Тут стояли самі регулярки по
    // тексту скрипта — і вони мовчки пройшли б, якби виняток для
    // взуття почав ковтати ще й сумки. Тому кличемо сам звіт.
    const { reportCardGaps } = require("../scripts/build-products.js");
    ...
    try {
        reportCardGaps([
            { category: "Кросівки", title: "Кросівки без габаритів", material: "Текстиль" },
            { category: "Жіночі сумки", title: "Сумка без габаритів", material: "Шкіра" },
            { category: "Кросівки", title: "Кросівки без матеріалу", dimensions: "—" }
        ]);
    } finally {
        console.warn = realWarn;
    }

    const proGabaryty = said.filter(line => line.includes("без габаритів")).join(" ");
    const proMaterial = said.filter(line => line.includes("без матеріалу")).join(" ");

    check("про габарити взуття не питає", !proGabaryty.includes("Кросівки"), proGabaryty);

    check("…а про габарити сумки питає",
        proGabaryty.includes("Жіночі сумки 1/1"), proGabaryty);

    // Виняток стосується САМЕ габаритів: матеріал у взуття є і в
    // фіді Google Shopping він окремий атрибут.
    check("матеріал у взуття питає далі",
        proMaterial.includes("Кросівки 1/2"), proMaterial);
}

console.log("\n[4a] Заповнене значення можна прочитати");
{
    // РЕГРЕСІЯ, ЯКУ ЦЕ ЛОВИТЬ. В окулярів Jimmy Choo габарити були
    // записані як «51» — у фіді це давало рядок «Розміри: 51», з
    // якого покупець не дізнається нічого: 51 чого і де?
    //
    // Правило про РЕЗУЛЬТАТ: у значенні мусить бути одиниця виміру
    // або підпис. Саме число — це не характеристика.
    const products = JSON.parse(read("data/products.json"));

    const nimi = products.filter(product => {
        const text = String(product.dimensions || "").trim();
        return text && !/(мм|см|діаметр)/i.test(text);
    });

    check(`габарити скрізь із одиницями (${products.length} товарів)`,
        nimi.length === 0,
        nimi.slice(0, 3).map(p => `${p.title}: ${JSON.stringify(p.dimensions)}`).join("; "));
}
