...
console.log("\n[6b] Під ціною в сітці немає порожнього відступу");
{
    ...
    const css = read("assets/css/style.css");
    // Порівнюємо з КОДОМ, а не з коментарями: на місці прибраного
    // правила лишилось пояснення, у якому це правило й наведене.
    ...
    // ПІДКРЕСЛЕННЯ НАЗВИ ПРИ НАВЕДЕННІ ТЕЖ ПРИБРАНО.
    //
    // Назва — справжнє посилання, і підкреслення було звичним сигналом
    // «сюди можна натиснути». Але на дворядковій назві це читалось як
    // дві смуги через півкартки, а сигнал і без нього є, причому
    // сильніший — див. перевірку нижче.
    check("назва не підкреслюється при наведенні",
        !/\.product-title-link:hover/.test(cssCode),
        (cssCode.match(/.*product-title-link:hover.*/) || [""])[0].trim());

    check("і без наведення теж",
        /\.product-title-link\{[^}]*text-decoration:none/.test(css));

    // ЩО ЛИШИЛОСЬ ЗАМІСТЬ. Прибрати сигнал взагалі було б погано:
    // людина не зрозуміла б, що картка клікабельна. Тому підйом усієї
    // картки мусить лишитись — він працює в обох режимах, і в сітці, і
    // в списку.
    check("картка при наведенні піднімається",
        /\.product-card:hover\{[\s\S]{0,120}transform:translateY\(-10px\)/.test(css));

    check("і робить це лише там, де наведення взагалі є",
        /@media \(hover:hover\) and \(pointer:fine\)\{\s*\.product-card:hover\{/.test(css));

    // Клавіатурний фокус НЕ залежав від :hover і мусить лишитись
    // видимим: інакше прибирання підкреслення зламало б доступність.
    check("клавіатурний фокус лишився видимим",
        /a:focus-visible,[\s\S]{0,300}outline:3px solid var\(--secondary\)/.test(css));
}
