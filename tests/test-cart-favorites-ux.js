...
console.log("\n[3] Підтвердження перед видаленням");
{
    ...
    // ОБРАНЕ ПИТАННЯ БІЛЬШЕ НЕ СТАВИТЬ — і це навмисно.
    //
    // Вікно «Видалити з обраного?» стояло ПЕРЕД дією, а видаляв
    // toggleFavorite() — перемикач. Поки вікно відкрите, рядок лишався
    // на екрані, тож другий клац по кнопці ДОДАВАВ товар назад.
    //
    // Тепер навпаки: видаляємо одразу, але 5 секунд це можна відкликати
    // кнопкою «Залишити товар». Дію видно, помилку виправити легше, ніж
    // раніше, і жодне повторне натискання нічого не повертає.
    check("обране не питає, а дає скасувати після",
        !/askConfirm\(\{[\s\S]{0,160}обраного\?/.test(common)
        && /startFavoriteRemoval\(/.test(common));

    check("вікно скасування — 5 секунд",
        /const FAVORITE_REMOVAL_DELAY = 5000/.test(favorites));

    check("є кнопка «Залишити товар»",
        /favorite-row-undo-cancel/.test(favorites)
        && /Залишити товар/.test(favorites));

    // Головне: видалення працює В ОДИН БІК. Саме перемикач у ролі
    // видалення й був причиною бага.
    check("видаляє removeFavorite, а не перемикач",
        /function removeFavorite/.test(common)
        && /removeFavorite\(id, \{ color, size \}\)/.test(favorites));

    check("повторний клац під час відліку нічого не робить",
        /if \(pendingRemovals\.has\(key\)\) return;/.test(favorites));

    // Пішли зі сторінки під час відліку — доводимо видалення до кінця,
    // інакше товар лишився б в обраному попри натиснуту кнопку.
    check("відлік не губиться при виході зі сторінки",
        /addEventListener\("pagehide"/.test(favorites));

    // На картці й сторінці товару те саме сердечко просто перемикає —
    // окремий сценарій потрібен лише в списку обраного.
    check("окремий сценарій тільки на сторінці обраного",
...
console.log("\n[4] Опис товару не дублюється");
{
    ...
    check("лишився саме розгортайний блок",
        // [^>]* — між класом і вмістом можуть стояти атрибути.
        // Тут з'явився data-product-description: обробник перемикання
        // кольору мусить знайти САМЕ опис, бо .spec-plain у
        // характеристиках чотири і перший з них — артикул.
        // Сама вимога не змінилась: опис живе в розгортайному блоці,
        // а не дублем зверху (перевірка нижче).
