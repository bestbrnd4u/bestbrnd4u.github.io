...
console.log("\n[5] Замовлення з САЙТУ теж сповіщаються");
{
  // ЩО ЗМІНИЛОСЬ. Раніше тут стояло «немає чату — виходимо», і
  // покупець із сайту не дізнавався ні про відправлення, ні про
  // накладну, ні про скасування. Тепер замість виходу — лист
  // (див. tests/test-customer-mail.js).
  //
  // Правило лишилось те саме: у Telegram пишемо ЛИШЕ за наявності
  // чату. Замовлення з сайту в чат не полізе.
  check("у чат пишемо лише за наявності чату",
        /const chatId = order\?\.telegram_chat_id;[\s\S]{0,900}if \(!chatId\) \{/.test(SRC)
        && /const text = customerStatusMessage/.test(SRC));

  check("замовленню з сайту йде лист",
        /if \(!chatId\) \{[\s\S]{0,400}sendCustomerMail/.test(SRC));

  // Власник мусить бачити правду: раніше йому радили дзвонити навіть
  // тоді, коли покупця вже повідомили.
  check("власнику видно, чим саме повідомили покупця",
        SRC.includes("notifiedVia") && SRC.includes("надіслано клієнту"));

  check("порада дзвонити лишилась тільки для невдачі",
        !SRC.includes("замовляв на сайті — передайте номер")
        && SRC.includes("Повідомити клієнта не вдалося"));
...
console.log("\n[11] Команда /ttn — не залежить від натиснутих кнопок");
{
  ...
  check("команда з @іменем бота працює (важливо для груп)",
        p("/ttn@bestbrnd4u_bot 0708553442 20450912345678")?.orderNumber === "0708553442");
