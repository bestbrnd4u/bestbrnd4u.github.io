...
console.log("\n[3] Інструкція повна");
{
  ...
  check("вказана роль Write як рекомендована",
        /Write/.test(page) && /Рекомендована роль/i.test(copy));
  check("сказано, що запрошення треба прийняти", /прийняти/i.test(copy));
  ...
  check("пояснено, що Read увійти не зможе", /Read .{0,120}не зможе/i.test(copy));
  check("сказано, що прав «тільки товари» не буває",
        /Окремих прав/i.test(copy));
}
