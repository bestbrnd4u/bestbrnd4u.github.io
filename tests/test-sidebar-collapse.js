...
console.log("\n[7] Смуга брендів — фільтр, а не перелік посилань");
{
    ...
    // СМУГА — НА ВИМОГУ, А НЕ ЗАВЖДИ.
    //
    // Спершу вона малювалась у каталозі щоразу, і в розділах теж.
    // Вийшло навпаки задуманому: людина відкриває «Новинки» подивитись
    // товари, а над ними стоїть перелік із двадцяти однієї назви й
    // відсуває каталог униз.
    //
    // Тепер її просить позначка brands=1, і ставить цю позначку рівно
    // один пункт меню — «Усі бренди» в ховері розділу. Тому перевіряємо
    // ОБИДВІ адреси: без позначки смуги бути не повинно.
    window.history.replaceState(null, "", "/catalog");
    window.__t.fillBrandStrip();

    check("без позначки смуга не малюється",
        chips().length === 0 && strip.hidden,
        chips().length + " плашок, hidden=" + strip.hidden);

    window.history.replaceState(null, "", "/catalog?brands=1");
    window.__t.fillBrandStrip();
