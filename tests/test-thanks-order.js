...
console.log("\n[7] Сторінка каже покупцеві, що лист прийде ЙОМУ");
{
    // ЩО БУЛО НЕ ТАК. Стояло «Ми надіслали деталі вашого замовлення
    // на НАШУ пошту» — тобто сторінка повідомляла покупцеві, що
    // магазин написав сам собі.
    //
    // А лист покупець отримує: «Замовлення №… прийнято», і далі на
    // кожну зміну статусу (mail.js в edge-функції). Питання, яке
    // людина ставить собі на цьому екрані, одне: «воно пройшло, мені
    // щось прийде?» Стара відповідь звучала як «ні».
    const html = read("thanks.html");

    check("про пошту магазину більше не пишемо",
        !/на нашу пошту/.test(html));

    check("сказано, що підтвердження вже в покупця",
        /Підтвердження вже надіслано на вашу пошту/.test(html));

    // Лист від незнайомого домену регулярно осідає в спамі, і людина
    // вирішує, що замовлення не пройшло.
    check("попереджено про «Спам»", /Спам/.test(html));

    check("і про листи на кожну зміну статусу",
        /зміну\s+статусу/.test(html));

    // Обіцянка мусить виконуватись завжди, інакше вона гірша за
    // мовчання. Пошта на оформленні обов'язкова — саме тому можна
    // писати без «якщо ви вказали пошту».
    const checkout = read("assets/js/checkout.js");

    check("пошта на оформленні справді обов'язкова",
        /REQUIRED_FIELDS = \[[^\]]*"email"/.test(checkout));

    // І лист справді існує — не обіцяємо того, чого немає.
    const mail = read("supabase/functions/telegram-order-bot/mail.js");

    check("лист «прийнято» справді надсилається",
        /subject: `Замовлення \$\{number\} прийнято`/.test(mail));

    check("і листи про статуси теж",
        /Замовлення \$\{number\} відправлено/.test(mail)
        && /Замовлення \$\{number\} виконано/.test(mail));

    // Менеджер дзвонить — про це теж лишилось сказано, бо для
    // частини покупців це головна обіцянка.
    check("про дзвінок менеджера не забули",
        /менеджер зв'яжеться з вами/.test(html));
}
