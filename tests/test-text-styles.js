...
console.log("\n[6] Стилі мають запасні значення");
{
    ...
    // --blk-title-base і --blk-text-base сюди не рахуємо, і це не
    // послаблення правила, а його зворотний бік.
    //
    // Решта змінних приходять З АДМІНКИ: їх може не бути, і тоді
    // блок мусить виглядати як раніше — тому запасне значення
    // обовʼязкове. А база розміру оголошується в тому самому правилі,
    // що й використовується:
    //
    //     .deal-head-text h2{
    //         --blk-title-base:30px;
    ...
    //     }
    //
    // Зникнути вона не може, а запасне значення тут уже одного разу
    // зіграло проти нас: саме «1em» у ролі запасного й ховало те, що
    // бази немає ніде, поки заголовок акції малювався 16-м кеглем
    // замість 42-го.
    const fromBlock = /--blk-(title|text)-base/;

    const noFallback = vars.filter(v => !v.includes(",") && !fromBlock.test(v));

    check(`у кожної змінної з адмінки є запасне значення (${vars.length})`,
    ...
    // Розмір — множник від ВЛАСНОГО розміру блока, а не від 1em і не
    // фіксоване число. Що кожен блок справді називає свою базу,
    // перелічено поіменно в tests/test-block-styles-scope.js [6a];
    // тут стережемо саму форму запису.
    check("розмір заголовка — множник від власної бази",
        /font-size:calc\(var\(--blk-title-base\) \* var\(--blk-title-scale, 1\)\)/.test(css)
        && !/var\(--blk-title-base, 1em\)/.test(css));

    check("розмір опису — так само",
        /font-size:calc\(var\(--blk-text-base\) \* var\(--blk-text-scale, 1\)\)/.test(css));
}
