...
console.log("\n[6] Зменшені копії зроблені з поточної бази");
{
    ...
    // ПОРЯДОК: СПОЧАТКУ 4:5, ПОТІМ КОПІЇ.
    //
    // Перевірка форми вище лагодить хибні копії НАСТУПНИМ прогоном, і
    // саме тому лишалась непоміченою причина: копії робились раніше за
    // приведення. Нове фото 1900×1900 давало за один прогін квадратні
    // avif при вертикальній базі — normalize() перероблює тільки webp.
    // Між двома збірками сайт стояв із обрізаними фото.
    //
    // Дивимось на порядок у main(): нормалізація мусить іти ДО циклу
    // buildMissingVariants. Коментарі відкидаємо — нижче пояснено
    // старий порядок, і без цього перевірка ловилася б на тексті.
    const mainFn = normalizer
        .slice(normalizer.indexOf("async function main()"))
        .replace(/\/\*[\s\S]*?\*\//g, "")
        .split("\n").filter(line => !/^\s*\/\//.test(line)).join("\n");

    // Дивимось і на порядок, і на УМОВУ, за якою приведення працює:
    // саме лише «normalize() стоїть вище» проходило б і тоді, коли той
    // блок вимкнений — а фото лишалось би квадратним.
    const умова = mainFn.indexOf("if (apply && off.length) {");
    const приведення = mainFn.indexOf("await normalize(info.file)");
    const копії = mainFn.indexOf("await buildMissingVariants(file)");

    check("приведення до 4:5 йде ДО побудови копій",
        умова > 0 && приведення > умова && копії > приведення,
        `умова: ${умова}, normalize: ${приведення}, variants: ${копії}`);

    // Один прогін — один результат. Якщо після build:media лишається
    // робота на другий раз, значить порядок знову розʼїхався.
    check("нормалізація робиться один раз, а не двічі",
        (mainFn.match(/await normalize\(/g) || []).length === 1,
        String((mainFn.match(/await normalize\(/g) || []).length));

    // Читання копії для перевірки форми — З ПАМʼЯТІ.
    //
    // sharp(шлях) тримає файл відкритим, а за кілька рядків ми пишемо
    // writeFileSync у той самий файл. На Windows це «UNKNOWN: unknown
    // error, open …-600.webp», і збірка падає на першому ж фото, якому
