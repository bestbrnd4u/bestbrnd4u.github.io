...
function decidedPhotosStayPut() {
    ...
    // [4], а не [2d]: перевірка асинхронна й друкується ПІСЛЯ [3].
    console.log("\n[4] Повторна збірка не псує фото з рішенням адмінки");
...
console.log("\n[2d] Звіт власникові каже правду");
{
    // ЩО БУЛО.
    //
    // Скрипт має ДВА різні шляхи, і один рядок звіту обслуговував
    // обидва:
    //
    //   заливка від краю — знає колір тла й допуск, а її share це
    //   частка кадру, ЯКУ ЗАЛИЛИ;
    //
    //   вирізання маскою — не знає ні кольору, ні допуску (там немає
    //   ні того, ні іншого), а його share це частка кадру, ЯКУ
    //   ЗАЙМАЄ ТОВАР.
    //
    // Тобто те саме число в цих двох випадках означає протилежне, а
    // двох полів у другому випадку просто немає. Для вирізаного фото
    // друкувалось:
    //
    //   «тло null → біле (59% кадру, допуск undefined)»
    //
    // Заміряно 21.09.2026 на rtlafd950301_33497204_4_v1_2x.webp.
    // null і undefined власник читає як поломку — і має рацію, бо
    // відсоток там ще й читається навпаки.
    const block = script.match(/if \(touched <= 10\) \{[\s\S]*?\n        \}\n/);

    check("рядок звіту знайдено", Boolean(block));

    if (block) {

        const say = new Function("file", "result", "let out = null;"
            + block[0].replace("if (touched <= 10) {", "{")
                      .replace(/console\.log\(/g, "out = (")
            + "\nreturn out;");

        // Вирізання маскою: ні кольору тла, ні допуску не існує.
        const cut = say("a.webp", { done: 59, bg: null, cut: true });

        check("вирізання: без null у звіті", !/null/.test(cut), cut);
        check("вирізання: без undefined у звіті", !/undefined/.test(cut), cut);

        // І сказано саме те, що сталось: 59% — це товар, а не заливка.
        check("вирізання: відсоток підписаний як товар",
            /товар займає 59% кадру/.test(cut), cut);

        // Заливка: обидва поля є й мусять лишитись у звіті.
        const fill = say("b.webp", { done: 34, bg: 238, tol: 14 });

        check("заливка: колір тла на місці", /238/.test(fill), fill);
        check("заливка: допуск на місці", /допуск 14/.test(fill), fill);

        check("заливка: відсоток підписаний як залите",
            /залито 34% кадру/.test(fill), fill);

        // Два шляхи — два різні рядки. Якщо вони колись знову
        // зіллються в один, цей набір це помітить.
        check("формулювання різні", cut !== fill);

    }

    // І сама позначка, за якою шляхи розрізняються, теж на місці.
    check("вирізання позначається cut: true",
        /return \{ done: Math\.round\(cut\.share \* 100\), bg: null, cut: true \};/.test(script));
}
