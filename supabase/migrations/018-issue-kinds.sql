...
-- з'явились три джерела, яким теж треба щось сказати власнику, — і
-- всі три мовчки відкидались.
--
...
--    адмінку.
--
-- 3. mail_list — ПІДПИСКА НЕ ПРОЙШЛА.
--
--    Форма підписки на листи додає людину в MailerLite через нашу
--    функцію. Якщо ключ став недійсним, форма мовчки перестає
--    працювати: людина бачить «спробуйте пізніше» й іде, а список не
--    росте. Дізнатись про це нізвідки.
-- --------------------------------------
...
--
-- Тому додаємо рівно три різновиди й лишаємо перевірку на місці.
-- --------------------------------------
...
begin
    ...
    -- ЩО ЗМІНИЛОСЬ: до переліку додано meta_capi, stock_out і
    -- mail_list.
    --
    ...
    -- сховищем.
    if v_kind not in ('js_error', 'not_found', 'meta_capi', 'stock_out', 'mail_list') then
        return;
    end if;
