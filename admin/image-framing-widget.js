...
(function () {
    ...
    // Розбір фону й заливка живуть у admin/white-preview.js.
    //
    // ЧОМУ НЕ ТУТ. Той самий результат потрібен картці праворуч
    // (admin/preview-templates.js): вона показувала файл як є, і фон у
    // ній лишався сірим, коли в рядку ліворуч уже був білий. Друга
    // копія заливки розійшлася б із цією на першій же правці — і два
    // вікна поруч показували б різне.
    //
    // Порожній результат тут не аварія: віджет мусить працювати й без
    // цього файлу (з тієї ж причини, що описана вище про ImageFraming —
    // інакше товар просто не збережеться).
    function whiteLib() {
        return window.WhitePreview || null;
    }
    ...
    var FrameEditor = createClass({
        ...
        getInitialState: function () {
            ...
            return {
                ...
                bgUniform: false,
                bgIsWhite: false,
                // Пікселі й адреси, за якими рахували, тут не тримаємо:
                // копія ImageData на кожен рядок — це мегабайти в стані
                // ні за чим, а адреси лежать на самому компоненті
                // (this.bgFor, this.whiteFor), бо звірятись із ними
                // треба ДО того, як setState встигне застосуватись.
                //
                // відбілене фото і причина, чому його немає
                whitePreview: null,
                bgWhy: null
            };
        },

        // Розбір фону запускаємо тут.
        //
        ...
        // а весь блок про фон (кружечок кольору, підпис, три кнопки й
        ...
        // рядка розбір фону так і не стартував би для щойно доданого
        ...
        // Фон фото: визначаємо колір і показуємо, яким він стане.
        //
        // ЧОМУ ПОКАЗУЄМО, А НЕ ПРОСТО ПЕРЕМИКАЄМО
        // ----------------------------------------
        // Заміна фону необоротна для файлу: після збірки оригінал уже
        ...
        //
        // Саму заливку рахує admin/white-preview.js — тим самим
        // алгоритмом і з тими самими числами, що збірка. Це не «схоже
        // на результат», а він і є.
        detectBackground: function () {
            ...
            if (this.bgFor === url) return;

            this.bgFor = url;

            white.analyze(url, function (found) {

                if (self.alive === false || self.bgFor !== url) return;

                // Фото не прочиталось — блок про фон просто не зʼявиться,
                // і це правильно: краще без нього, ніж із вигаданим
                // кольором. Але слід у консолі лишаємо: мовчазна
                // відсутність цілого блоку інтерфейсу — саме те, через
                // що його одного разу вже не помітили.
                if (!found) {
                    console.warn("Кадрування: не вдалось розібрати фон фото:", url);
                    return;
                }

                self.setState({
                    bgColors: found.colors,
                    bgColor: found.color,
                    bgUniform: found.uniform,
                    bgIsWhite: found.isWhite
                });
        ...
        //
        // ПОКАЗУЄМО НЕ ЛИШЕ ПРИ НАТИСНУТІЙ КНОПЦІ
        // ----------------------------------------
        // Раніше відбілене фото зʼявлялось тільки після «Зробити білим».
        // Але збірка вирівнює фон і САМА — коли він однорідний і не
        // білий. Тобто найчастіший випадок («нічого не натискав») якраз
        // і показувався неправдиво: у рядку сіре фото, після публікації
        // біле.
        //
        // Тепер питаємо white-preview, що станеться з цим файлом, і
        // показуємо саме це. Рішення (порядок перевірок) там спільне зі
        // збіркою, тож «показане» і «опубліковане» — те саме.
        //
        // І РАХУЄМО ОДИН РАЗ
        // -------------------
        // Колись заливка + toDataURL проганялись прямо в render(), тобто
        // на кожен рух повзунка й на кожен піксель перетягування точки.
        // Тепер результат лежить у стані, а white-preview ще й тримає
        // спільний кеш на всю адмінку: те саме фото просять рядок
        // кадрування, велике фото в картці й мініатюра під ним.
        ensureWhitePreview: function () {
            ...
            // Ключ — адреса ПЛЮС рішення: натиснули «Не чіпати» —
            // відповідь інша, хоча фото те саме.
            var key = url + "|" + ((this.props.frame && this.props.frame.bg) || "авто");

            if (this.whiteFor === key) return;

            // Ключ тримаємо на компоненті, а не в стані: setState не
            // встигає застосуватись до синхронної відповіді (рішення
            // «не чіпати» відомо без читання файлу), і перевірка нижче
            // відкинула б власний же результат.
            this.whiteFor = key;

            white.resolve(url, this.props.frame, function (result) {

                if (self.alive === false || self.whiteFor !== key) return;

                self.setState({ whitePreview: result.url, bgWhy: result.why });
        ...
        contentBounds: function (img) {
            ...
            // Колір фону беремо з КАДРУ, а не з зашитого числа.
            //
            // ЧОМУ. Тут стояв поріг 244: «світліше — значить білий фон».
            ...
            //
            // Розбір той самий, що й для заливки: одна функція на
            // обидва місця (admin/white-preview.js) — інакше «межа
            // товару» і «межа фону» рано чи пізно розійшлись би.
            var white = whiteLib();

            if (!white) return null;

            var found = white.borderColors(data, w, h);
        ...
        render: function () {
            ...
            return h("div", {
                ...
                h("div", {
                    ...
                    url
                        ? h("img", {
                            // Показуємо ПЕРЕМАЛЬОВАНЕ фото, а не оригінал:
                            // рішення приймається, коли результат видно.
                            //
                            // Умови «якщо натиснуто біле» тут немає
                            // навмисно — white-preview віддає картинку
                            // рівно тоді, коли фон зміниться при
                            // публікації, зокрема й без жодної кнопки.
                            src: this.state.whitePreview || url,
                            style: imageStyle,
                            draggable: false,
                            alt: ""
                        })
                ...
                h("div", { className: "framing-controls" },
                    ...
                    this.state.bgColor
                        ? h("div", { className: "framing-bg" },
                            ...
                            // Підказка мусить збігатися з тим, що на
                            // екрані. Раніше вона виправдовувалась —
                            // «у картці праворуч фон ще старий, він
                            // зміниться при публікації», — і це читалось
                            // як «кнопка не спрацювала». Тепер картка
                            // показує той самий результат, і виправдання
                            // не потрібне.
                            h("p", { className: "framing-hint" },
                                !this.state.bgUniform
                                    ? "Фото на моделі або в інтерʼєрі — фон тут замінити не можна."
                                    // Вирізання робить нейромережа під
                                    // час збірки; у браузері її немає,
                                    // тож показати результат тут
                                    // неможливо. Про це вже сказано під
                                    // кнопкою «Вирізати товар», і
                                    // повторювати іншими словами не варто.
                                    : frame.bg === "cutout"
                                        ? "Кадр і наближення діють і для вирізаного фото."
                                        : frame.bg === "keep"
                                            ? "Це фото лишиться як є."
                                            : this.state.bgIsWhite && frame.bg !== "white"
                                                ? "Нічого робити не потрібно."
                                                : this.state.whitePreview
                                                    ? (frame.bg === "white"
                                                        ? "Так фото виглядатиме після публікації — і тут, і в картці праворуч."
                                                        : "Збірка вирівняє фон сама — результат уже показано. «Не чіпати», щоб залишила як є.")
                                                    // Мовчазне «нічого не
                                                    // змінилось» — саме те,
                                                    // через що кнопка одного
                                                    // разу вже виглядала
                                                    // зламаною. Тому причину
                                                    // називаємо вголос.
                                                    : this.state.bgWhy
                                                        ? "Фон лишиться як є: " + this.state.bgWhy + "."
                                                        : "Рахуємо, яким стане фон…"))
                        : null,
