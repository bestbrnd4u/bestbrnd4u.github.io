...
(function () {
    ...
    function renderDetail() {
        ...
        detailBox.innerHTML = `
            ...
            ${order.comment ? `
            <!-- Побажання покупця — ОКРЕМИМ розділом, а не рядком
                 серед реквізитів доставки. Це єдине місце замовлення,
                 де людина написала щось своїми словами, і пропустити
                 його дорожче за все інше на цій картці. -->
            <div class="section">
                <h3>Побажання покупця</h3>
                <p class="comment">${esc(order.comment)}</p>
            </div>` : ""}
