...
function main() {
    ...
    // ЧОМУ ТУТ БІЛЬШЕ НЕМА ПЕРЕЙМЕНУВАННЯ ЗА ВМІСТОМ
    //
    // Був механізм: реєстр відбитків data/image-fingerprints.json, і
    // якщо вміст файлу змінився — файлу дописували відбиток у імʼя, щоб
    // кеш віддав нову картинку. Ідея правильна, реалізація — надто
    // крихка, і це виявилось на першому ж бойовому запуску.
    //
    // Перейменування вимагає, щоб УСІ посилання переписались разом із
    // файлом: у data/products/*.json, products.json, image-variants.json,
    // розмітці, CSS. Досить одному місцю розійтися — і фото просто
    // зникає зі сайту. Саме так і сталося: файл переїхав, а посилання
    // лишились на старому імені.
    //
    // А ціна помилки несиметрична. Застарілий кеш — це «людина бачить
    // попередню картинку кілька годин». Розʼїхане посилання — «товар
    // без фото». Друге значно гірше, тож механізм, який ризикує другим
    // заради першого, не виправданий.
    //
    // Кеш картинок вирішується інакше — версією в адресі, яку додає
    // збірка до згенерованих даних (scripts/build-products.js). Адреса
    // файлу при цьому не змінюється, тож розійтися нічому.
    //
    // Перейменування задовгих імен (вище) лишається: воно потрібне
    // через обмеження файлової системи, спрацьовує рідко й одразу, при
    // завантаженні файлу.
