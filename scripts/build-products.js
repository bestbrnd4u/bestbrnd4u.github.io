...
const PRODUCT_PAGE_ONLY = [
    ...
    "dimensions",
    "strapInfo",
    // Застібка, відділення й декор потрапили сюди 24.09.2026, коли їх
    // дозаповнили більшості товарів. Доти вони були майже порожні, і
    // їхня відсутність у переліку нічого не коштувала: шість значень
    // на сотню товарів.
    //
    // Тепер це 61, 39 і 20 заповнених полів — близько 5 КБ тексту,
    // який їде в КОЖЕН список: каталог, головну, кошик, пошук,
    // обране. А показує його лише сторінка товару (product.js) і фід
    // Google Shopping (build-feed.js), і обидва читають products.json,
    // а не полегшений каталог.
    "closure",
    "compartments",
    "decor",
    "instagramBlock",
...
function main() {
    ...
    parsed.forEach(({ file, filePath, data }) => {
        ...
        // ЗАЙВИЙ ПРОБІЛ У НАЗВІ БРЕНДУ — ЦЕ ДРУГИЙ БРЕНД.
        //
        // «Invicta » і «Invicta» для JavaScript — різні рядки, і товар
        // із хвостом випадає з кожного підрахунку, який групує за
        // назвою. Видно це було так: на /brands/ смуга показувала
        // «Invicta 0» і «Adidas by Stella Mccartney 0», хоча в каталозі
        // ті самі бренди рахувались як 1. Причина — два джерела назви:
        // хаб бере її з групування (а там .trim()), смуга рахує за
        // сирим полем товару. Два написання не сходились, і рівно два
        // товари зі 131 не потрапляли в жодну плашку.
        //
        // Обхідні .trim().toLowerCase() по коду вже стояли — у логотипів
        // і в аліасах, із коментарем «у даних трапляється Invicta з
        // хвостом». Але кожен НОВИЙ споживач мусив би про них згадати,
        // і перший же не згадав. Тому чистимо один раз тут, у джерелі:
        // далі всі бачать одне написання, і обхідні шляхи більше нічого
        // не рятують — вони просто не потрібні.
        const brandAsWritten = String(data.brand || "");
        const brandClean = brandAsWritten.replace(/\s+/g, " ").trim();

        if (brandAsWritten !== brandClean) {

            data.brand = brandClean;

            console.log(`   ↻ ${file}: зайві пробіли в назві бренду — «${brandClean}»`);
