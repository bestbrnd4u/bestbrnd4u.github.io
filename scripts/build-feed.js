...
// показу.
//
// СЕЙЛ, ЯКИЙ УЖЕ СКІНЧИВСЯ, — НЕ СЕЙЛ.
//
// Заміряно у фіді 25.09.2026. Товар 57 (The J Marc Shoulder Bag Green
// Glow) ніс таке:
//
//     g:price       9000.00 UAH
//     g:sale_price  8600.00 UAH
//     g:sale_price_effective_date  …-09-13/…-09-18
//
// Вікно скінчилось 18 вересня, тобто за тиждень до збірки, а акційна
// ціна лишалась у фіді. На самому сайті все правильно: сторінка
// товару показує 9000, бейджа «SALE» немає, розмітка Product теж
// каже 9000, а сторінка акції відповідає «її вже завершено». Тобто
// фід був єдиним місцем, де ціна, якої вже не існує, ще жила.
//
// Google за такого вікна показує звичайну ціну, тож покупець поганої
// ціни не побачив. Але вище в цьому ж файлі написано, чого ми тут
// боїмось: будь-яка розбіжність між фідом і сторінкою — це причина
// зняти товар із показу. Протермінований сейл нічого не дає й чекає
// лише на те, щоб хтось колись не віддав вікно.
//
// Беремо ЛИШЕ половину правила про «to». Повне saleActive() із
// common.js тут не годиться: воно віддає false і ДО початку сейлу, а
// майбутнє вікно ми якраз і хочемо віддати Google заздалегідь — саме
// заради цього все це й задумано.
function saleEnded(sale, now) {

    if (!sale || !sale.to) return false;

    const ends = new Date(sale.to).getTime();

    // Дата, яку не вдалось розібрати, нічого не вимикає: краще
    // лишити як було, ніж прибрати ціну через одруківку в даті.
    if (!Number.isFinite(ends)) return false;

    return (Number.isFinite(now) ? now : Date.now()) >= ends;
...
// заповнено в адмінці. На 24.09.2026: розміри (91), матеріал (103),
// ремінь (60), застібка (61), відділення (39), склад (1).
//
// Числа виросли після того, як характеристики дозаповнили з
// офіційних сторінок брендів — доти застібка стояла в п'яти товарів,
// а відділень не було взагалі, і тримати їх тут не було сенсу.
//
// Порядок — від найчастішого питання покупця: «який розмір», «з чого
// зроблено», «як носити», «що туди влізе».
...
function highlightsOf(product) {
    ...
    const rows = [
        ...
        // Відділення — це відповідь на «що туди влізе», і для сумки
        // чи гаманця вона важить не менше за розмір. З 39 заповнених
        // у межу 150 символів не влазить рівно одне — гаманець Coach
        // із вісьмома відділеннями для карток; його обріже трикрапкою
        // нижче, і це прийнятно.
        ["Відділення", product.compartments]
    ];
    ...
    return rows
        .map(([label, value]) => {
            ...
            if (full.length <= HIGHLIGHT_LIMIT) return full;

            // Ріжемо по межі слова, а не посеред нього: «…та ще двома
            // відділеннями для карток; п…» виглядає як збій, а не як
            // скорочення. Якщо пробілу поблизу немає (одне довге
            // слово), лишаємо як було — різати нíде.
            const cut = full.slice(0, HIGHLIGHT_LIMIT - 1);
            const space = cut.lastIndexOf(" ");

            return `${(space > HIGHLIGHT_LIMIT - 40 ? cut.slice(0, space) : cut).trimEnd()}…`;
