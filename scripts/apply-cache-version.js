...
function main() {
    ...
    htmlFiles().forEach(file => {
        ...
        //
        // Коли товар змінює адресу, build-product-pages.js лишає за
        // старою адресою заглушку: canonical, meta refresh, location
        // .replace() і рядок тексту. Вона не вантажить ні стилів, ні
        // скриптів, ні даних — перевірено, жодна з 73 таких сторінок
        // не згадує assets/ взагалі.
        //
        // А отже, рядок із версіями там нікому читати. Він лише:
        //   • додає 668 байтів до сторінки вагою 606 — тобто подвоює
        //     її, і цю вагу качає кожен, хто прийшов за старим
        //     посиланням;
        //   • змушує всі 73 заглушки перезаписуватись щоразу, коли
        //     змінився будь-який файл даних, — і вони засмічують
        //     кожну збірку, хоч самі не змінились ні на символ.
        ...
        assets.forEach(({ rel, hash }) => {
            ...
            //
            // КОРОТКА ФОРМА НЕ МАЄ ПРАВА НА ПРЕФІКС ТЕКИ — і це не
            // дрібниця синтаксису.
            //
            // В admin/ і в assets/js/ є однойменні файли: promo.js,
            // reviews.js. Поки коротка форма «promo.js» шукалась із
            // будь-яким префіксом, вона знаходила ще й
            // "assets/js/promo.js" на сторінці акції — і ставила туди
            // відбиток АДМІНСЬКОГО файлу.
            //
            // Наслідок тихий і довгий: сайтовий promo.js мінявся, а
            // адреса лишалась тією самою, бо admin/promo.js не чіпали.
            // Браузер і Cloudflare місяцями віддавали стару версію.
            // Заміряно 12.09: у гілці лежав promo.js з відбитком
            // 78acc2bf, а сторінка просила 417e4a55 — відбиток
            // admin/promo.js, — і Cloudflare віддавав копію віком
            // 10,5 години. Виглядало так, ніби налаштування в адмінці
            // не діють.
            const forms = rel.startsWith("admin/")
                ? [{ form: rel, anyDir: true }, { form: rel.slice("admin/".length), anyDir: false }]
                : [{ form: rel, anyDir: true }];

            forms.forEach(({ form, anyDir }) => {

                const escaped = form.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");

                // Повний шлях може стояти і в абсолютній адресі
                // (https://…/assets/js/app.js) — там префікс потрібен.
                // Короткий — лише одразу після лапки.
                const prefix = anyDir ? "(?:[^\"']*\\/)?" : "";

                html = html.replace(
                    new RegExp(`(["'])(${prefix}${escaped})(?:\\?v=[a-f0-9]+)?\\1`, "g"),
                    `$1$2?v=${hash}$1`
                );
