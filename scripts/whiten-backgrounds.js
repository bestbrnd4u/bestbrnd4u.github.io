...
async function main() {
    ...
    for (const file of files) {
        ...
        if (touched <= 10) {
            ...
            // ДВА ШЛЯХИ — ДВА ЗВІТИ, І ПЛУТАТИ ЇХ НЕ МОЖНА.
            //
            // Заливка від краю знає колір тла й допуск, і її share —
            // це частка кадру, ЯКУ ЗАЛИЛИ.
            //
            // Вирізання маскою не знає ні кольору, ні допуску (там
            // немає ні того, ні іншого), а його share — це частка
            // кадру, ЯКУ ЗАЙМАЄ ТОВАР. Тобто те саме число означає в
            // цих двох випадках протилежне.
            //
            // Один спільний рядок друкував для вирізання
            // «тло null → біле (59% кадру, допуск undefined)»:
            // два порожні поля й відсоток, який читається навпаки.
            const percent = result.done || result.would;

            console.log(result.cut
                ? `   ${file}: тло вирізано моделлю → біле (товар займає ${percent}% кадру)`
                : `   ${file}: тло ${result.bg} → біле (залито ${percent}% кадру,`
                    + ` допуск ${result.tol})`);
