...
// МАСКУ БЕРЕМО НЕ ЯК Є, А З ПОРОГОМ
//
// ЩО БУЛО НЕ ТАК (сумка Jacquemus, крупний план)
// -----------------------------------------------
// Вихід мережі — карта «наскільки це схоже на товар», розтягнута на
// 0..255. Її підставляли прозорістю просто так: piksel = товар·a +
// біле·(1−a). Там, де мережа вагається (a ≈ 0.5), від пікселя лишалась
// половина, змішана з білим.
//
// На предметному фото з великим фоном це майже не помітно: мережа
// впевнена і в товарі, і в фоні. А на крупному плані, де сумка займає
// половину кадру, вагань багато — і темно-коричнева шкіра вийшла
// молочною, ніби за матовим склом. Власник описав це як «сумку
// выбелело».
//
// Розподіл маски на тому знімку по децилях:
//
//   0–10%  34.6%   ← певне тло
//   10–60%  8.5%   ← вагання, і саме вони псували товар
//   60–100% 56.8%  ← певний товар
//
// Тепер нижче LOW — чисте тло, вище HIGH — недоторканий товар, між
// ними мʼякий перехід на кілька відсотків маски. Край лишається
// згладженим (там ще й blur у productMask), а сам товар більше не
// підмішується до білого.
const ALPHA_LOW = 0.25;
const ALPHA_HIGH = 0.55;

function alpha(a) {

    if (a <= ALPHA_LOW) return 0;

    if (a >= ALPHA_HIGH) return 1;

    return (a - ALPHA_LOW) / (ALPHA_HIGH - ALPHA_LOW);
...
async function cutoutToWhite(file) {
    ...
    for (let p = 0; p < width * height; p++) {
        ...
        const a = alpha(mask.data[p * mask.stride] / 255);
...
module.exports = { cutoutToWhite, productMask, alpha, ALPHA_LOW, ALPHA_HIGH, MODEL };
