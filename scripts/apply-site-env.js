...
// ---- 3. robots.txt ----
// robots.txt збираємо ЗАНОВО, а не правимо наявний.
//
// ЩО БУЛО НЕ ТАК
// ---------------
// Тут стояло редагування тексту: вирізати блок регулярним виразом,
// дописати інший. На кожній збірці залишалися порожні рядки від
// вирізаного, а правила дописувалися ще раз. За кілька десятків
// збірок файл розпух до 428 рядків: сотні порожніх, правила
// повторені тричі й маркери конфлікту злиття посередині.
//
// Пошуковик такий файл читає до першої незрозумілої стрічки, тож
// частина правил могла просто не діяти — а помітити це можна лише
// відкривши файл очима.
//
// Тепер вміст не залежить від того, що лежало у файлі раніше: він
// повністю визначається середовищем. Немає що накопичувати.
rewrite("robots.txt", () => {
    ...
    return [
        ...
        "Allow: /",
        "",
        "# Службові сторінки: індексувати нічого, вміст у них особистий",
        "Disallow: /admin/",
        "Disallow: /account",
        "Disallow: /cart",
        "Disallow: /checkout",
